<div id=toc></div>

# Table of Contents

- [immunology](#immunology) [Total: 1]
- [EGFR Antibody Therapeutics](#EGFR Antibody Therapeutics) [Total: 15]
- [CEACAM5 Cancer](#CEACAM5 Cancer) [Total: 4]
- [EGFR Target Biology](#EGFR Target Biology) [Total: 18]
- [CEACAM5 Antibody Therapeutics](#CEACAM5 Antibody Therapeutics) [Total: 4]


<div id='immunology'></div>

# immunology [[Back]](#toc)

### [1] [Simultaneous TCR and IL-2 agonism selectively enhances epitope-specific CD8 T cell responses during chronic viral infection](https://www.biorxiv.org/content/10.64898/2026.04.13.718150v1)
*Hashimoto, M.,Khan, M. A.,Akhtar, A.,Agrewala, J. N.,Freeman, G.,Girgis, N.,Zhang, Y.,Low, S.,Quayle, S. N.,Suri, A.,Ahmed, R.*

Source: bioRxiv

Main category: immunology

TL;DR: 本研究评估了Immuno-STAT平台在慢性LCMV感染小鼠模型中的效果。该平台通过融合肽-MHC复合物和减毒IL-2突变体，实现对抗原特异性CD8+ T细胞的选择性激活。结果显示，Immuno-STAT能特异性扩增靶向T细胞并增强其效应功能，对非靶向细胞影响极小，且与PD-1阻断联用具有协同增效作用，为克服IL-2全身毒性及优化癌症免疫疗法提供了预临床依据。


<details>
  <summary>Details</summary>
Motivation: 白细胞介素-2（IL-2）虽能有效增强抗原特异性CD8+ T细胞反应，但其严重的全身毒性限制了临床应用。现有工程化IL-2疗法试图改变受体偏向性或靶向递送，但理想策略应是仅向抗原特异性CD8+ T细胞选择性传递信号，以促进其效应分化同时 sparing 非靶向群体。本研究旨在解决这一核心挑战，验证Immuno-STAT平台在慢性感染背景下实现TCR与IL-2信号协同刺激的选择性和有效性。

Method: 研究采用慢性淋巴细胞脉络膜脑膜炎病毒（LCMV）感染的小鼠模型。使用针对DbGP33-41表位的Immuno-STAT融合蛋白进行治疗，该蛋白包含二价肽-MHC I类复合物和亲和力衰减的IL-2突变体。研究对比了靶向（DbGP33特异性）与非靶向（DbGP276特异性）CD8+ T细胞的扩增情况及表型变化，并进一步评估了Immuno-STAT与PD-1阻断抗体联合使用的治疗效果。

Result: 1. Immuno-STAT治疗选择性扩增了DbGP33特异性CD8+ T细胞，使其呈现效应样表型。
2. 非靶向的DbGP276特异性CD8+ T细胞几乎无扩增，证实了该疗法的高度选择性；仅在非靶向细胞中观察到CD25和CX3CR1表达轻微增加，提示存在旁观者IL-2信号。
3. Immuno-STAT与PD-1阻断联用比单独使用PD-1阻断更有效地增强了DbGP33特异性CD8+ T细胞的反应，且对非靶向群体影响甚微。

Conclusion: Immuno-STAT平台能够通过在慢性抗原暴露期间协调TCR特异性和IL-2信号，选择性地增强靶向病毒特异性CD8+ T细胞的功能，同时最小化对非靶向群体的激活。结合PD-1阻断可产生协同效应。这些发现为开发下一代IL-2疗法提供了机制理解和预临床支持，表明将TCR特异性与IL-2信号整合是推进癌症免疫治疗和慢性感染治疗的有效策略。

Abstract: Interleukin-2 (IL-2) remains an attractive cytokine for enhancing antigen-specific CD8 T cell responses in cancer immunotherapy, but systemic toxicity hinders its broad clinical application. To address this, various IL-2-based therapeutics have been engineered with altered IL-2 receptor bias or targeted delivery to tumors, the tumor microenvironment, or immune cell populations. Ideally, IL-2 signals should be selectively delivered to antigen-specific CD8 T cells, boosting their responses and promoting effector differentiation while sparing non-targeted populations. Immuno-STATTM (Selective Targeting and Alteration of T cells) is a fusion protein platform comprising a bivalent peptide-MHC class I complex and an affinity-attenuated IL-2 mutein that co-stimulates TCR and IL-2 signaling in epitope-specific CD8 T cells. Here, we investigated whether a DbGP33-41-targeted Immuno-STAT enhances DbGP33-specific CD8 T cell responses in a mouse model of chronic lymphocytic choriomeningitis virus (LCMV) infection. Immuno-STAT treatment selectively expanded DbGP33-specific CD8 T cells with an effector-like phenotype. Non-targeted DbGP276-specific CD8 T cells showed little to no expansion in response to DbGP33-41-targeted Immuno-STAT therapy, underscoring the selectivity of this approach. However, minor changes in phenotypic markers, including increased expression of CD25 and CX3CR1, were observed in non-targeted CD8 T cells, likely reflecting bystander IL-2 signaling. Combining Immuno-STAT with PD-1 blockade augmented DbGP33-specific CD8 T cell responses more effectively than PD-1 blockade alone, with minor effects on the non-targeted DbGP276-specific population. These findings inform the clinical development of Immuno-STAT and other IL-2 therapeutics and highlight the value of coordinated TCR and IL-2 stimulation during chronic antigen exposure, alone or in combination with PD-1 blockade. IMPORTANCEInterleukin-2 (IL-2) is a key cytokine for promoting effector differentiation of antigen-specific CD8 T cells and remains an attractive agent in cancer immunotherapy, but systemic toxicity limits its clinical use. This study addresses a central challenge in IL-2-based immunotherapy: delivering IL-2 to cognate antigen-specific CD8 T cells while minimizing activation of non-targeted populations. Using a mouse model of chronic lymphocytic choriomeningitis virus (LCMV) infection, we show that the Immuno-STAT (Selective Targeting and Alteration of T cells) platform selectively expands targeted virus-specific CD8 T cells and enhances their function while limiting effects on non-targeted populations. We also show that combining Immuno-STAT with PD-1 blockade further enhances targeted virus-specific CD8 T cell responses during chronic LCMV infection. These findings provide mechanistic and preclinical support for integrating T cell receptor (TCR) specificity with IL-2 signaling to advance cancer immunotherapy and guide next-generation IL-2 therapeutics for cancer and chronic infection.

</details>


<div id='EGFR Antibody Therapeutics'></div>

# EGFR Antibody Therapeutics [[Back]](#toc)

### [2] [Molecular Targeting of EGFR, BRAF, and HER2 Signaling in Colorectal Cancer: Contemporary Advances with Panitumumab, Encorafenib, and Tucatinib.](https://pubmed.ncbi.nlm.nih.gov/41899309/)
*Piotr Kawczak,Tomasz Bączek*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: 本文综述了转移性结直肠癌（mCRC）中针对EGFR、BRAF和HER2信号通路的靶向治疗进展，强调了基于RAS、BRAF V600E、HER2扩增及错配修复状态等生物标志物的精准治疗策略，包括帕尼单抗、恩考芬尼和图卡替尼等药物的临床应用及耐药机制。


<details>
  <summary>Details</summary>
Motivation: 转移性结直肠癌（mCRC）是全球癌症相关死亡的主要原因之一。随着分子谱分析技术的进步，基于特定基因改变（如RAS、BRAF、HER2等）的生物标志物驱动治疗策略已成为改善患者预后的关键，亟需系统总结当前靶向治疗的证据以优化临床实践。

Method: 本文采用叙述性综述方法，系统评估了关键的II期和III期临床试验、转化医学研究以及真实世界数据，重点分析针对EGFR、BRAF和HER2通路的靶向疗法在mCRC中的疗效、安全性及患者选择标准。

Result: 1. 对于RAS/BRAF野生型左侧肿瘤，抗EGFR疗法（如帕尼单抗联合化疗）相比抗血管生成方案显著提高了反应率和总生存期。
2. 对于预后较差的BRAF V600E突变肿瘤，恩考芬尼联合EGFR阻断剂显示出具有临床意义的生存获益。
3. 在HER2扩增的mCRC中，曲妥珠单抗联合方案及选择性酪氨酸激酶抑制剂（如图卡替尼）在经治患者中表现出持久的缓解和良好的安全性。

Conclusion: 基于EGFR、BRAF和HER2的靶向治疗显著扩展了mCRC分子定义亚组的治疗选择。未来的精准肿瘤学方向应侧重于生物标志物指导的患者筛选、深入理解耐药机制以及开发新兴的联合治疗策略，以进一步改善患者结局。

Abstract: Metastatic colorectal cancer (mCRC) remains a major cause of cancer-related mortality worldwide. Advances in molecular profiling have transformed the therapeutic landscape, enabling biomarker-driven treatment strategies based on alterations in RAS, BRAF V600E, HER2 amplification, and mismatch repair status. Among these, dysregulation of the epidermal growth factor receptor (EGFR), BRAF, and HER2 signaling pathways represents a central driver of tumor progression and therapeutic resistance. Targeted agents directed against these pathways-including the anti-EGFR monoclonal antibody panitumumab, the selective BRAF inhibitor encorafenib, and the HER2-selective tyrosine kinase inhibitor tucatinib-have substantially expanded treatment options for molecularly defined subgroups of patients with mCRC. Anti-EGFR therapy remains a cornerstone of treatment for patients with RAS/BRAF wild-type, left-sided tumors. Panitumumab combined with chemotherapy has demonstrated significant improvements in response rates and overall survival compared with anti-angiogenic-based regimens in randomized clinical trials. For tumors harboring BRAF V600E mutations, which are associated with poor prognosis, combination strategies incorporating encorafenib with EGFR blockade have shown clinically meaningful survival benefits and represent an important therapeutic advance. In HER2-amplified colorectal cancer, HER2-targeted therapies have emerged as an effective treatment strategy. Trastuzumab-based combinations and HER2-selective tyrosine kinase inhibitors such as tucatinib have demonstrated durable responses and favorable safety profiles in heavily pretreated patients. This review summarizes current evidence from pivotal phase II and III clinical trials, translational studies, and real-world data evaluating EGFR-, BRAF-, and HER2-directed therapies in colorectal cancer. Particular emphasis is placed on biomarker-guided patient selection, mechanisms of resistance, and emerging combination strategies that continue to refine precision oncology approaches in mCRC.

</details>


### [3] [Long-term efficacy and safety of pegunigalsidase alfa administered every 4 weeks in adults with Fabry disease: results from up to 5 years of the BRIGHT F51 phase III, open-label extension study.](https://pubmed.ncbi.nlm.nih.gov/41862950/)
*Myrl Holida,Aleš Linhart,Nicola Longo,Eric Wallace,Camilla Tøndel,Derralynn Hughes,David G Warnock,Antonio Pisani,François Eyskens,Patrick Deegan,Ulla Feldt-Rasmussen,Ozlem Goker-Alpan,Ankit Mehta,Giovanni Piotti,Vito Fichera,Meng Wang,Raul Chertkoff,Stephen Waldek,William R Wilcox,John A Bernat*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: BRIGHT F51研究的 interim 分析显示，法布雷病患者从每两周一次的传统酶替代疗法转换为每四周一次的 Pegunigalsidase alfa 治疗后，在3-5年的长期随访中保持了疾病稳定且耐受性良好，但抗药物抗体（ADA）阳性男性的肾功能下降速率略高，需进一步监测。


<details>
  <summary>Details</summary>
Motivation: 目前获批的法布雷病酶替代疗法（ERT）需每两周输注一次（E2W）。Pegunigalsidase alfa 是一种聚乙二醇化（PEGylated）的 ERT，具有更长的半衰期，理论上可将给药间隔延长至每四周一次（E4W），从而提高患者依从性和治疗便利性。本研究旨在评估这种延长给药间隔方案在长期治疗中的有效性和安全性。

Method: 这是一项正在进行的多中心、开放标签、III期延伸研究（BRIGHT F51, NCT03614234）。研究对象为成年法布雷病患者，此前已接受 agalsidase alfa 或 beta E2W 治疗至少3年，并在 BRIGHT 研究中完成了一年的 Pegunigalsidase alfa 治疗。本中期分析报告了截至2022年12月31日，患者接受 Pegunigalsidase alfa 2 mg/kg E4W 治疗3-5年后的数据。主要评估指标包括年化 eGFR 斜率、血浆 lyso-Gb3 浓度变化以及不良事件（特别是输注相关反应和抗药物抗体 ADA 的发生情况）。

Result: 共纳入29名患者。治疗期间年化 eGFR 斜率中位数为 -2.2 mL/min/1.73 m²/年；其中男性为 -2.4，女性为 -1.8。ADA 阳性患者（均为男性）的 eGFR 下降速率（-2.6）高于 ADA 阴性患者（-1.8）。男性患者血浆 lyso-Gb3 从基线到第208周的中位数变化为 3.2 nM，而女性患者浓度保持低且稳定。总体安全性良好，45%的患者出现与治疗相关的不良事件（均为轻度/中度），31%的患者经历轻度/中度输注相关反应。仅1名患者出现 transient de novo ADA。

Conclusion: 长期每四周一次给予 Pegunigalsidase alfa 2 mg/kg 治疗耐受性良好，并能维持疾病稳定，尤其在女性和 ADA 阴性男性患者中效果显著。然而，ADA 阳性男性的临床结局仍需更多数据以充分理解。建议在 E4W 治疗期间密切监测临床结果，最终研究结果将进一步验证该给药方案的可行性。

Abstract: BACKGROUND: Enzyme replacement therapies (ERTs) approved for Fabry disease require infusions every 2 weeks (E2W). Pegunigalsidase alfa, a PEGylated ERT with a prolonged half-life vs. other ERTs, may allow extension of the dosing interval to every 4 weeks (E4W). BRIGHT F51 (NCT03614234) is an ongoing phase III, open-label extension study evaluating long-term efficacy and safety of pegunigalsidase alfa 2 mg/kg E4W in adults with Fabry disease previously treated with agalsidase alfa or beta E2W for ≥ 3 years who completed one year of pegunigalsidase alfa treatment in the BRIGHT study. This interim analysis reports results following 3–5 years of treatment (cutoff date December 31, 2022).

RESULTS: Twenty-nine patients were enrolled. Median (interquartile range [IQR]) annualized eGFR slope during treatment was ‒2.2 (‒2.9; ‒1.1) mL/min/1.73 m2/year (males: ‒2.4 [‒2.9; ‒1.0, n = 23]; females: ‒1.8 [‒2.4; ‒1.3, n = 6]; anti-drug antibody [ADA]-positive: ‒2.6 [‒4.0; ‒1.7, n = 9 all male]; ADA-negative: ‒1.8 [‒2.7; ‒0.6, n = 20]). Median (IQR) change in plasma lyso-Gb3 from baseline to Week 208 was 3.2 (‒3.9; 8.5, n = 17) nM in males; concentrations remained low and stable in females. Overall, 51/477 treatment-emergent adverse events in 13 patients (45%) were considered treatment-related (all mild/moderate). Nine patients (31%) experienced mild/moderate infusion-related reactions. One patient developed transient de novo ADAs.

CONCLUSIONS: Long-term treatment with pegunigalsidase alfa 2 mg/kg E4W was well-tolerated and maintained disease stability, especially in females and ADA-negative males; more data are needed to better understand outcomes in ADA-positive males. Clinical outcomes should be closely monitored during E4W treatment. The final results of this extension study will further assess the feasibility of this dosing regimen.

TRIAL REGISTRATION DETAILS: ClinicalTrials.gov, NCT03614234. Registered July 30, 2018; https://clinicaltrials.gov/study/NCT03614234.

GRAPHICAL ABSTRACT: [Image: see text]

SUPPLEMENTARY INFORMATION: The online version contains supplementary material available at 10.1186/s13023-026-04303-8.

</details>


### [4] [Diabetes Mellitus Attenuates Response to Anti-CD20 Therapy in Primary Membranous Nephropathy Despite Equivalent B-Cell Depletion.](https://pubmed.ncbi.nlm.nih.gov/41860432/)
*Xiao-Meng Xu,Meng-Ting Wu,Fang Zhao,Ji-Peng Li,Ye Chen,Rui Lu,Yang Zha,Yu-Ting Zhang,Hong-Bin Peng,Hai-Chao Cao,Li-Jie He*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: 本研究通过回顾性队列分析发现，尽管合并糖尿病（DM）的原发性膜性肾病（MN）患者与非糖尿病患者在接受抗CD20单抗治疗后B细胞清除率相当，但其临床缓解率显著较低。研究建议对这类患者加强血糖控制或调整免疫抑制方案以改善预后。


<details>
  <summary>Details</summary>
Motivation: 原发性膜性肾病（MN）与糖尿病（DM）常共存，但DM对抗CD20单克隆抗体治疗MN疗效的具体影响尚不明确。研究者假设，尽管B细胞清除效果可比，但合并DM的MN患者相比非DM患者，其治疗反应会降低。

Method: 这是一项回顾性队列研究，纳入接受至少一个完整疗程抗CD20抗体治疗的成年原发性MN患者。采用倾向评分匹配（1:2），基于年龄、性别、基线eGFR、尿蛋白和PLA2R抗体状态进行匹配。主要结局为3、6、9、12个月时的缓解情况（完全或部分缓解）；次要结局包括免疫学缓解、B细胞清除及肾脏参数变化。使用多变量逻辑回归、Kaplan-Meier分析及亚组分析比较MN+DM组与单纯MN组的结局。

Result: 匹配后共纳入207名患者（72例MN+DM，135例MN）。在12个月随访期内，两组的B细胞清除率和免疫学缓解率无统计学显著差异。然而，单纯MN患者在9个月（94% vs. 77%; OR=4.55, P<0.01）和12个月（97% vs. 86%; OR=5.26, P<0.05）的总体缓解率显著高于MN+DM组。亚组分析证实糖尿病患者在多个分层中反应均较差。两组安全性 profile 相似。

Conclusion: 尽管B细胞清除效果等效，但合并糖尿病的MN患者对抗CD20抗体治疗的反应低于非糖尿病患者。这表明严格的血糖控制、针对反应不佳者的早期补充抗CD20剂量以及联合免疫抑制方案可能有助于改善此类患者的临床结局。

Abstract: BACKGROUND AND HYPOTHESIS: Primary membranous nephropathy (MN) and diabetes mellitus (DM) are common comorbidities, yet the impact of DM on the response to anti-CD20 monoclonal antibody therapy in MN remains unclear. We hypothesized that despite comparable B-cell depletion, MN patients with comorbid DM would exhibit a reduced treatment response compared to non-diabetic MN patients.

METHODS: In this retrospective cohort study, we included adults with primary MN who received at least one complete course of anti-CD20 antibody therapy. Propensity score matching (1:2) was applied based on age, sex, baseline eGFR, urinary protein and PLA2R antibody status. The primary outcome was remission (complete or partial) at 3,6,9,12 months. Secondary outcomes included immunological remission, B-cell depletion, and changes in renal parameters. Multivariable logistic regression, Kaplan-Meier analysis, and subgroup analyses were used to compare outcomes between MN patients with (MN+DM) and without diabetes (MN).

RESULTS: After matching, 207 patients were included (72MN+DM, 135MN). B-cell depletion and immunological remission rates showed no statistically significant differences between groups over the 12-month follow-up period. However, MN patients had significantly higher overall remission rates at 9 months (94% vs. 77%; OR=4.55, 95% CI=1.52-12.50, P<0.01) and at 12 months (97% vs. 86%; OR=5.26, 95% CI=1.25-20.00, P<0.05). Subgroup analyses confirmed consistently poorer responses in diabetic patients across multiple strata. Safety profiles were similar between groups.

CONCLUSION: Despite equivalent B-cell depletion, MN with DM patients show a reduced response to anti-CD20 antibody therapy relative to non-diabetic patients, underscoring that tight glycemic control, early supplementary anti-CD20 dosing for poor response, and combined immunosuppressive regimens may enhance their clinical outcomes.

</details>


### [5] [Orthogonal Tri-Modular Coiled-Coil Assembly for Programmable Multi-Cargo Display on Escherichia coli Nissle 1917.](https://pubmed.ncbi.nlm.nih.gov/41860170/)
*Yong Joon Cho,Gyu-Jin Lee,Jong-Ha Park,Sung In Lim*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: 本研究开发了基于大肠杆菌Nissle 1917的TriSCs（三特异性支架细胞），利用正交可编程的卷曲螺旋相互作用实现表面功能模块的动态、可逆及多重组装，显著增强了抗癌活性并支持实时生物成像，为精准诊疗提供了新型活体生物药平台。


<details>
  <summary>Details</summary>
Motivation: 现有的活体生物药（LBPs）表面功能化策略主要依赖静态基因融合或共价修饰，限制了多重化能力和动态重构能力，难以实现位点特异性干预的灵活性和多功能集成。

Method: 构建基于大肠杆菌Nissle 1917的TriSCs平台，在细菌外膜展示三种不同的α-螺旋基序，形成可重构的生物界面。通过卷曲螺旋相互作用，采用“混合即用”（mix-and-go）策略，实现多种功能模块（如DR5激动性纳米抗体、EGFR靶向纳米抗体及荧光报告分子）的自发、高特异性且可逆的组装。

Result: 空间共展示DR5激动性纳米抗体和EGFR靶向纳米抗体相比可溶性组合产生了增强的抗癌活性，证实了表面组织化信号传导的优势；同时，模块化整合荧光报告分子实现了实时生物成像。

Conclusion: TriSCs通过将正交性、可逆性和多重化能力集成于单一活体底盘，建立了一个动态且可编程的LBP平台，为下一代生物工程治疗剂和精准诊疗应用奠定了基础。

Abstract: Live biotherapeutic products (LBPs) represent emerging living medicines capable of site-specific intervention; however, current strategies for surface functionalization rely predominantly on static genetic fusion or covalent modification, limiting multiplexing and dynamic reconfiguration. Here, we engineer TriSCs (Tri-specific Scaffold Cells), a tri-modular and orthogonally programmable living scaffold based on Escherichia coli Nissle 1917. Three distinct α-helical motifs are displayed on the bacterial outer membrane, forming a reconfigurable biointerface that enables spontaneous, highly specific, and reversible payload assembly via coiled-coil interactions. This mix-and-go strategy allows simultaneous and selective recruitment of multiple functional modules without structural interference. Spatial co-display of a DR5 agonistic nanobody and an EGFR-targeting nanobody produces enhanced anticancer activity compared to soluble combinations, underscoring the impact of surface-organized signaling. In parallel, modular incorporation of a fluorescent reporter enables real-time bioimaging. By integrating orthogonality, reversibility, and multiplex capability within a single living chassis, TriSCs establish a dynamic and programmable LBP platform for precision theranostics and next-generation bioengineered therapeutics.

</details>


### [6] [Characterization of HER2-Positive Murine Breast Cancer Models for Investigating HER2-Targeted Therapy and Immunotherapy.](https://pubmed.ncbi.nlm.nih.gov/41899598/)
*Yun Lu,Benjamin P Lee,Abbigael V Eli,Shannon E Lynch,Ar Rafi Md Faisal,Jonathan Moye,Anna G Sorace*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: 本研究构建了具有自发脑转移能力的HER2阳性同基因小鼠乳腺癌模型（4T1和EO771系），评估了其对靶向治疗、免疫检查点阻断及抗体药物偶联物（ADC）的反应，为研究HER2+脑转移及联合疗法提供了有价值的临床前工具。


<details>
  <summary>Details</summary>
Motivation: HER2阳性乳腺癌患者总体生存率较低且脑转移风险高，但当前缺乏稳健的HER2+转移性同基因小鼠模型来研究靶向治疗和免疫调节疗法的效果。

Method: 通过慢病毒转导将人HER2基因（野生型或突变型p.A775_G776insYVMA，C端标记GFP）引入具有已知转移潜力的三阴性乳腺癌细胞系（4T1和EO771）。利用Western blot和免疫组化分析HER2表达及磷酸化水平。使用曲妥珠单抗、图卡替尼、免疫检查点抑制剂（抗PD-1/抗CTLA-4）及ADC药物治疗肿瘤，并通过脑部荧光成像评估转移潜能及治疗 efficacy。

Result: 新建细胞系均表达HER2+，其中突变型磷酸化水平更高。4T1-HER2体内成瘤率为100%，EO771-HER2为15-30%。HER2过表达使4T1模型自发脑转移增加30%。曲妥珠单抗单药未缩小原发灶，但显著降低脑部GFP信号。在EO771-HER2YVMA模型中，抗HER2疗法与免疫检查点阻断联合使用有效抑制肿瘤生长并延长生存期。T-Dxd在EO771-HER2WT模型中显示部分疗效，而T-DM1无效。

Conclusion: 成功开发了能自发脑转移且对免疫疗法和ADC呈现差异化反应的HER2+同基因肿瘤模型。这些模型对于推进HER2+脑转移分子成像、研究药物血脑屏障穿透性以及探索包括免疫疗法在内的联合治疗策略具有重要价值。

Abstract: Background/Objectives: Human epidermal growth factor receptor 2 (HER2)-positive breast cancer is linked to poorer overall survival and a higher risk of brain metastases compared to HER2-negative breast cancer. Current preclinical studies lack robust HER2+ metastatic syngeneic mouse models for investigating targeted and immunomodulatory therapies. This study aims to develop effective HER2+ mouse models to investigate response dynamics to HER2-targeted therapy and immunotherapy. Methods: The human HER2 gene (WT or mutant p.A775_G776insYVMA, GFP-tagged at the C-terminus) was introduced into triple-negative breast cancer (TNBC) mouse mammary carcinoma cells with known metastatic potential (4T1 and EO771) via lentiviral transduction. HER2 expression and phosphorylation were analyzed using Western blotting and immunohistochemistry. Tumors were treated with HER2-targeted therapy (trastuzumab and tucatinib), immune checkpoint blockade (anti-PD-1 and anti-CTLA-4), and anti-HER2 antibody-drug conjugate (ADC) to evaluate treatment efficacy. Metastatic potential was assessed with brain fluorescence imaging. Statistical analysis included ANOVA and Kaplan-Meier tests. Results: Newly established lines demonstrated expression of HER2+, with HER2YVMA lines showing higher phosphorylation than HER2WT lines. Cells were tumorigenic, demonstrating in vivo tumor take rates at 100% for 4T1-HER2 and 15-30% for EO771-HER2. HER2 overexpression led to a 30% increase in spontaneous brain metastasis in the 4T1-HER2 models. Trastuzumab alone did not reduce primary tumor size but significantly reduced brain GFP signal by 17% ± 8% and 26% ± 7% in the 4T1-HER2WT and 4T1-HER2YVMA models, respectively. Combinational therapies with anti-HER2 therapy and immune checkpoint blockade effectively suppressed primary tumor growth and prolonged survival in EO771-HER2YVMA model. T-Dxd, but not T-DM1, demonstrated partial treatment response in the EO771-HER2WT model. Conclusions: HER2+ syngeneic tumor models were developed that spontaneously metastasize to the brain and demonstrate variable responses to immunotherapies and ADCs. These models are valuable for advancing molecular imaging modalities for HER2+ brain metastasis, studying blood-brain barrier penetration of HER2-targeted drugs, and exploring the combination of therapies, including immunotherapy.

</details>


### [7] [Oncolytic adenoviruses encoding bispecific T cell engagers or a novel trispecific T cell engager for dual-targeting of c-MET and EGFR.](https://pubmed.ncbi.nlm.nih.gov/41552759/)
*Martin A Boos,Oliver Seifert,Stefanie Sawall,Jessica Genz,Annika Huber,Ilse Hofmann,Roland E Kontermann,Guy Ungerechts,Dirk M Nettelbeck*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: 本研究开发了一种由溶瘤腺病毒（oAds）编码的双特异性或三特异性T细胞衔接器（TCEs），通过同时靶向EGFR和c-MET克服肿瘤异质性导致的免疫逃逸，其中基于双抗体的三特异性TCE（Db-TriTE）表现出最佳的基因组稳定性和疗效。


<details>
  <summary>Details</summary>
Motivation: 解决病毒免疫治疗中因肿瘤异质性导致的免疫逃逸问题，提高对表达不同靶点（EGFR和c-MET）的肿瘤细胞的杀伤效率。

Method: 1. 设计并表征针对不同格式的双特异性TCE（scDb-cMET和taFv-EGFR）及基于双抗体的三特异性TCE（Db-TriTE scDb-cMET-scFv-EGFR）。
2. 在体外评估TCE诱导细胞毒性的能力，特别是针对单阳性和双阳性细胞的效果。
3. 构建携带不同TCE表达策略的溶瘤腺病毒（oAds）面板。
4. 通过感染肿瘤细胞与外周血单个核细胞（PBMCs）的共培养实验，评估病毒的转基因表达、T细胞激活及细胞毒性诱导能力。

Result: 1. 双特异性TCE在体外显示出有效性，促使了Db-TriTE的设计。
2. Db-TriTE展现出有效的双重靶向能力，对双阳性细胞具有更优的疗效。
3. 携带TCE转基因的oAds未出现明显的病毒衰减，并能特异性诱导T细胞激活和细胞毒性。
4. 编码Db-TriTE的oAds相比编码两个双特异性TCE的策略，具有更优越的基因组稳定性和整体最高疗效。

Conclusion: 研究报道了新型的双特异性和三特异性TCE，并证明了将其作为双重靶向病毒抗体疗法应用的原理可行性，其中Db-TriTE结合溶瘤腺病毒是克服肿瘤异质性的有效策略。

Abstract: This study reports a strategy to overcome immune escape of tumors during viro-immunotherapy due to tumor heterogeneity. We pursued a dual-targeting approach of epidermal growth factor receptor (EGFR) and cellular mesenchymal epithelial transition factor (c-MET) facilitated by two bispecific T cell engagers (TCEs) or one trispecific TCE encoded by oncolytic adenoviruses (oAds). Different bi- and trispecific TCE formats were generated and characterized. We showed efficacy of bispecific TCEs single-chain diabody (scDb)-cMET and tandem scFv (taFv)-EFGR in vitro, encouraging the design of the diabody-based trispecific TCE (Db-TriTE) scDb-cMET-scFv-EGFR. The Db-TriTE exhibited effective dual-targeting, i.e., the induction of cytotoxicity for single-positive cells of both targets and superior efficacy on double-positive cells. Therefore, a panel of oAds was produced to test different TCE (co-)expression strategies. Our results showed no or minor attenuation of viruses harboring the transgenes for one of the analyzed insertion strategies, expression of TCEs, and specific induction of both T cell activation and cytotoxicity in co-cultures of infected tumor cells with peripheral blood mononuclear cells (PBMCs). Importantly, we demonstrated dual-targeting by oAds encoding the two bispecific TCEs or the Db-TriTE with the latter featuring superior genomic stability and the overall highest efficacy. Taken together, we report novel bi- and trispecific TCEs and proof of principle for their application as dual-targeted viro-antibody therapy.

</details>


### [8] [Case Report: A case of severe hypotension induced by nimotuzumab in a nasopharyngeal carcinoma patient.](https://pubmed.ncbi.nlm.nih.gov/41929496/)
*Wenhui Liu,Ping Qin,Feiyao Xiao,Bao Sun*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: 本病例报告记录了一例晚期鼻咽癌患者在接受尼妥珠单抗联合放疗治疗后，出现罕见且严重的持续性低血压。排除其他诱因后确认为药物不良反应，经停药及去甲肾上腺素治疗后缓解，提示临床需加强血压监测以防范严重低血压风险。


<details>
  <summary>Details</summary>
Motivation: 尽管尼妥珠单抗安全性良好，其引起的低血压通常为轻微且可逆，但严重持续性低血压较为罕见。本文旨在通过具体病例警示临床医生关注这一潜在严重不良反应，弥补现有对尼妥珠单抗罕见副作用认知的不足。

Method: 采用病例报告方法，详细描述一例65岁男性晚期鼻咽癌患者在接受5个周期尼妥珠单抗联合放疗后的临床表现。通过排除心脏功能、内分泌功能、甲状腺功能、炎症因子及其他药物影响等混杂因素，确立低血压与尼妥珠单抗的因果关系，并记录停药及使用去甲肾上腺素升压的治疗过程。

Result: 患者在使用尼妥珠单抗后出现严重持续性低血压。在排除其他相关因素后，判定该不良反应与尼妥珠单抗密切相关。经停止用药并给予持续去甲肾上腺素治疗后，患者血压恢复稳定。

Conclusion: 虽然尼妥珠单抗相关的低血压大多轻微且可逆，但临床上仍存在发生严重低血压的风险。建议在临床实践中加强对患者的血压监测，保持警惕，以便及时发现并干预严重低血压事件。

Abstract: Nimotuzumab, a humanized immunoglobulin G1 monoclonal antibody, is mainly used in combination with radiotherapy for the treatment of stage III/IV nasopharyngeal carcinoma (NPC) or locally advanced head and neck squamous-cell carcinoma (HNSCC) with positive epidermal growth factor receptor (EGFR) expression. Although the drug has a favorable safety profile, it is associated with various adverse reactions, among which hypotension is relatively uncommon. Clinically, hypotension caused by nimotuzumab mainly manifests as mild blood pressure (BP) reduction, which can be relieved via rest. This case reports a 65-year-old male patient with advanced NPC who developed severe persistent hypotension after receiving radiotherapy combined with 5 cycles of nimotuzumab treatment. After excluding related factors such as cardiac function, endocrine function, thyroid function, inflammatory factors and other drugs, the adverse reaction was considered to be closely associated with nimotuzumab. After discontinuing the drug and giving continuous norepinephrine to increase BP, the patient's BP returned to stable. This case suggests that although nimotuzumab-related hypotension is mostly mild and reversible, BP monitoring should still be strengthened to maintain vigilance against severe hypotension and intervene promptly in clinical practice.

</details>


### [9] [AI-enhanced precision: boosting pathologist confidence in HER2 ultra-low diagnosis through intelligent re-screening.](https://pubmed.ncbi.nlm.nih.gov/41845525/)
*Xin-Ping Wang,Ying Han,Min Yao,Hong-Wen Gao,Ping-Li Sun*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: 本研究评估了AI工具在乳腺癌HER2超低表达诊断中的辅助作用。结果显示，AI显著提高了病理学家检测HER2超低的准确性（F1分数从0.78升至0.98）、一致性和效率，并减少了低估现象，有助于优化ADC疗法患者筛选。


<details>
  <summary>Details</summary>
Motivation: HER2免疫组化（IHC）表达的准确评估，特别是“HER2超低”状态的精确识别，对于指导曲妥珠单抗德鲁替康（T-DXd）等抗体药物偶联物（ADC）治疗至关重要。然而，HER2超低表达的判读存在困难且一致性较差，亟需辅助工具以提高诊断质量。

Method: 研究纳入188例乳腺癌病例（376张全切片图像），包括136对未接受新辅助治疗（NAT）的核心穿刺活检（CNB）与手术切除（SE）匹配样本，以及52对接受NAT后的CNB-SE匹配样本（HER2 IHC评分为0或1+）。使用Roche VENTANA DP 600扫描仪获取WSIs，并通过Roche Navify®数字病理平台进行AI辅助分析。以专家共识诊断为金标准，9名不同经验的病理学家在有/无AI辅助下重新评估幻灯片，并在包含89张幻灯片的外部队列中验证通用性。主要评估指标包括诊断准确性、效率、置信度及一致性。

Result: 在专家鉴定的92张HER2超低幻灯片中，AI辅助使病理学家的F1分数从0.78提升至0.98，精确率从0.39提升至0.91，召回率从0.72提升至0.96。在非NAT匹配对中，AI将CNB判读与SE金标准的一致性从0.37提高至0.48，主要减少了HER2低估；在NAT后SE标本中，分类一致性从0.63提高至0.92。此外，AI显著提高了病理学家间的诊断一致性，缩短了解读时间，并增加了高置信度评估的比例。

Conclusion: AI支持显著提升了在活检、切除及治疗后标本中识别HER2超低的准确性、一致性和效率。通过提供更精确和自信的诊断，AI辅助有助于改善新型ADC疗法的患者选择，具有显著的临床应用价值。

Abstract: Accurate assessment of Human Epidermal Growth Factor Receptor 2 (HER2) immunohistochemistry (IHC) expression, particularly the precise identification of "HER2 ultra-low" status, is critical for guiding antibody-drug conjugate (ADC) therapies, such as trastuzumab deruxtecan (T-DXd) in breast cancer. Given the difficulty and poor consistency in interpreting HER2 ultra-low expression, this study evaluated an artificial intelligence (AI) tool for assisting pathologists in diagnosing HER2 ultra-low expression in breast cancer. The analysis included 188 breast cancer cases (376 whole-slide images), consisting of 136 matched core needle biopsy (CNB)-surgical excision (SE) pairs from patients without neoadjuvant therapy (NAT) and a separate cohort of 52 matched CNB-post-NAT SE pairs with HER2 IHC scores of 0 or 1+. All slides were scanned using the Roche VENTANA DP 600 scanner. The resulting whole-slide images (WSIs) were subsequently analyzed and scored for HER2 IHC expression using AI assistance via the Roche Navify® Digital Pathology platform. Expert consensus diagnosis served as the ground truth. Nine pathologists of varying experience re-evaluated their assigned slides with and without AI assistance, assessing changes in diagnostic accuracy, efficiency, and confidence. Additionally, an external cohort of 89 slides from two independent hospitals was used to validate the generalizability of the AI-assisted HER2 interpretation. Among 92 HER2 ultra-low slides (40 biopsy and 52 SE sections) identified by experts, AI assistance significantly improved pathologists' detection performance, increasing the F1-score from 0.78 to 0.98, precision from 0.39 to 0.91, and recall from 0.72 to 0.96 against expert consensus. In non-NAT matched pairs, AI improved the concordance between CNB interpretation and SE ground truth from 0.37 to 0.48, primarily by reducing HER2 underestimation. In post-NAT SE specimens, AI increased classification consistency from 0.63 to 0.92. Furthermore, AI assistance significantly elevated diagnostic agreement among pathologists, reduced interpretation time, and increased the proportion of high-confidence assessments. These findings demonstrate that AI support enhances the accuracy, consistency, and efficiency of HER2 ultra-low identification across biopsy, resection, and post-treatment specimens, potentially improving patient selection for novel ADC therapies. With AI support, pathologists can deliver more precise and confident diagnoses of HER2 ultra-low breast cancer.

</details>


### [10] [The value of time-dependent diffusion MRI in nasopharyngeal carcinoma: correlation with prognostic factors.](https://pubmed.ncbi.nlm.nih.gov/41844850/)
*Haoran Wei,Fan Yang,Sai Wang,Xiaolu Li,Xiaoduo Yu,Lin Li,Yanfeng Zhao,Yueluan Jiang,Meng Lin,Hongmei Zhang*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: 本研究评估了时间依赖性扩散磁共振成像（td-dMRI）参数与鼻咽癌（NPC）预后因素的相关性。结果显示，td-d衍生的微观结构参数（如细胞外扩散率、细胞内体积分数、直径和细胞密度）及不同频率下的表观扩散系数（ADC）与PD-L1、EGFR、Ki-67、TNM分期及EBV状态显著相关。其中，td-dMRI测量的肿瘤直径与病理测量值高度相关（r=0.772）。结论表明，td-dMRI能无创反映肿瘤微观结构，具有用于NPC风险分层和治疗监测的潜力。


<details>
  <summary>Details</summary>
Motivation: 旨在探索时间依赖性扩散磁共振成像（td-dMRI）作为一种无创影像学手段，能否通过提取微观结构参数来反映鼻咽癌（NPC）的肿瘤微环境特征，并评估这些参数与关键预后因素（如分子标志物、TNM分期、EBV状态等）之间的相关性，从而为临床风险分层和治疗监测提供新的影像学生物标志物。

Method: 前瞻性纳入116名受试者（105例NPC患者，11例良性增生）。获取四个td-dMRI衍生的微观结构参数：细胞外扩散率（Dex）、细胞内体积分数（Vin）、直径（Diameter）和细胞密度（Cellularity），以及三个振荡频率下的表观扩散系数（ADC0Hz, ADC30Hz, ADC55Hz）。记录TNM分期及各项预后指标，将连续变量按最佳截断值二分化。采用相关性分析、组间比较及受试者工作特征（ROC）曲线分析（以曲线下面积AUC评估诊断效能）来总结数据。

Result: 1. **分子标志物关联**：PD-L1与ADC值相关（ADC55Hz的AUC为0.708）；EGFR与细胞密度和直径相关；Ki-67表达与Dex和ADC55Hz相关。
2. **组织学与分期**：Vin、细胞密度和ADC0/30Hz与组织学亚型相关（Vin的AUC最高，为0.706）；TNM分期与直径和细胞密度相关。
3. **鉴别诊断**：ADC30Hz区分早期NPC（T1-2）与增生的效能最佳（AUC=0.847）。
4. **EBV相关**：多个参数与EBV抗体呈负相关；诱导化疗免疫治疗后，Vin和ADC0/30Hz在EBV DNA清除组间存在显著差异。
5. **病理验证**：td-dMRI测量的直径与病理测量直径高度相关（r=0.772, p<0.001）。

Conclusion: td-dMRI参数能够反映鼻咽癌的肿瘤微观结构特征，并与多种预后因素显著相关。该技术提供了一种无创表征肿瘤微结构的方法，在鼻咽癌患者的风险分层、疗效监测及管理策略优化方面具有潜在的临床应用价值。

Abstract: OBJECTIVES: To evaluate the correlation between time-dependent diffusion MRI (td-dMRI) parameters and prognostic factors in nasopharyngeal carcinoma (NPC).

MATERIALS AND METHODS: 116 patients (105 NPC, 11 benign hyperplasia) were prospectively enrolled. Four td-dMRI-derived microstructural parameters, extracellular diffusivity (Dex), intracellular volume fraction (Vin), Diameter, and Cellularity, and apparent diffusion coefficient (ADC) at three oscillation frequencies (ADC0Hz, ADC30Hz, and ADC55Hz), were obtained. TNM stages and prognostic factors were recorded; continuous variables were dichotomized by optimal cut-offs. Correlations, between-group comparisons, and receiver operating characteristic analysis summarized diagnostic performance as the area under the curve (AUC).

RESULTS: Programmed death ligand 1 was correlated with ADC values, with ADC55Hz achieving an AUC of 0.708. Epidermal growth factor receptor was correlated with Cellularity and Diameter (r = 0.341 and -0.329). Ki-67 expression was associated with Dex and ADC55Hz (r = 0.252 and 0.286). Vin, Cellularity and ADC0/30Hz were linked to histological subtype, with Vin achieving the highest AUC of 0.706. TNM stages were correlated with Diameter (r = 0.203 to 0.371) and Cellularity (r = -0.365 to -0.284). ADC30Hz best distinguished NPC (T1-2) from hyperplasia (AUC = 0.847). Negative correlations were observed between several parameters and Epstein-Barr virus (EBV)-based antibodies (r = -0.269 to -0.239). Vin and ADC0/30Hz differed across EBV DNA clearance groups after induction chemoimmunotherapy (p ≤ 0.038). td-dMRI-measured diameters correlated with pathological measurements (r = 0.772, p < 0.001).

CONCLUSIONS: td-dMRI parameters may reflect tumor microstructure and show associations with prognostic factors in NPC, suggesting potential utility for risk stratification and treatment monitoring.

KEY POINTS: Question Can microstructural parameters from time-dependent diffusion MRI (td-dMRI) noninvasively provide clinically meaningful prognostic information for nasopharyngeal carcinoma? Findings Differences in prognostic factor levels can be reflected by td-dMRI, and td-dMRI-derived diameter was correlated with pathology-derived diameter (r = 0.772, p < 0.001). Clinical relevance td-dMRI provides a noninvasive method to characterize tumor microstructure, potentially improving patient management strategies in nasopharyngeal carcinoma.

</details>


### [11] [Evaluation of HER2 status in endometrial carcinoma using endometrial, gastric, and breast cancer criteria: focus on HER2-low.](https://pubmed.ncbi.nlm.nih.gov/41906783/)
*Heesoo Yoon,Yeon Bi Han,Dong Hoon Suh,Kidong Kim,Jae Hong No,Yong Beom Kim,Hyojin Kim*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: 本研究评估了365例子宫内膜癌（EC）中HER2的状态，发现HER2阳性与浆液性组织学、高级别及p53异常亚组密切相关。HER2-low在高风险患者中具有独立的预后价值，且肿瘤间存在显著异质性，支持将HER2-low作为抗体药物偶联物（ADC）的治疗靶点，并强调在复发或转移时需重新评估HER2状态。


<details>
  <summary>Details</summary>
Motivation: 旨在通过三种不同标准评估子宫内膜癌中的HER2状态，分析其与临床病理及分子特征的关联，探究肿瘤间异质性及其预后意义，特别是关注HER2-low亚组的临床价值。

Method: 对365例EC全切片进行HER2免疫组化染色，分别采用EC、胃癌（GC）和乳腺癌（BC）标准判读；对2+/3+病例进行银增强原位杂交检测；并在30对匹配的原发与复发/转移肿瘤中评估肿瘤间异质性。

Result: HER2阳性率为2.0%，显著关联浆液性组织学、高级别及p53异常亚组。HER2-low检出率因标准而异（EC标准45.7%，GC/BC标准18%），并与晚期及高级别肿瘤相关。HER2表达富集于p53异常、高/进阶风险及FIGO III-IV期肿瘤。肿瘤间异质性显著（40.0%/30.0%），尤其在浆液性和高级别肿瘤中。基于GC/BC标准，HER2-low是高/进阶风险组复发的独立预后因素。

Conclusion: EC中HER2状态因评分标准不同而异，需标准化判读。HER2表达（含HER2-low）多见于生物学侵袭性强的亚型，且在高风险患者中具有预后相关性，提示其作为ADC治疗靶点的潜力。鉴于疾病进展中HER2状态的动态变化，建议在复发或转移病灶中重新评估。

Abstract: OBJECTIVE: To evaluate human epidermal growth factor receptor 2 (HER2) status in endometrial carcinoma (EC) using 3 different criteria, assess its association with clinicopathological and molecular characteristics, and investigate intertumoral heterogeneity and prognostic significance, focusing on HER2-low.

METHODS: HER2 immunohistochemistry was performed on whole-slide sections from 365 EC and interpreted using EC, gastric cancer (GC), and breast cancer (BC) criteria. Silver in situ hybridization was performed for 2+/3+ cases. Intertumoral heterogeneity was assessed in 30 matched primary and recurrent/metastatic tumors.

RESULTS: Among 365 ECs, HER2-positive tumors (2.0%) were strongly associated with serous histology, high-grade, and the p53-abnormal (p53-abn) subgroup (p<0.001). HER2-low (45.7% by EC criteria and 18% by GC/BC criteria) was associated with advanced stage (p=0.045) and high-grade tumors (p=0.009), respectively. HER2-expression was enriched in p53-abn (60.4%, 37.7%), high-/advanced-risk (56.2%, 60.0%), and International Federation of Gynecology and Obstetrics III-IV (60.0%, 62.5%) tumors. Intertumoral heterogeneity was substantial (40.0%, 30.0%), reflecting dynamic changes, particularly in serous histology (p=0.018) and high-grade tumors (p=0.034). Based on GC/BC criteria, HER2-low status was an independent prognostic factor for recurrence in high- and advanced-risk groups compared with HER2-0 (p=0.040).

CONCLUSION: HER2 status in EC varies across scoring criteria, supporting the need for standardized interpretation. HER2 expression, including HER2-low, was more prevalent in biologically aggressive subsets, and HER-low showed prognostic relevance in high-/advanced-risk patients, supporting its potential value as a therapeutic target for antibody-drug conjugate therapy. The dynamic nature of HER2 across disease progression further supports the need for reassessment in recurrent or metastatic lesions.

</details>


### [12] [First-line zolbetuximab plus mFOLFOX6 and nivolumab in unresectable CLDN18.2-positive gastric or gastroesophageal junction adenocarcinoma: a phase 2 trial.](https://pubmed.ncbi.nlm.nih.gov/41840238/)
*Kohei Shitara,Hirokazu Shoji,Nicola Fazio,Sara Lonardi,Keun-Wook Lee,Li-Yuan Bai,Kensei Yamaguchi,Jean-Philippe Metges,Gianluca Masi,Denis Smith,Tae-Yong Kim,Maria Matsangou,Archita Shrivastava,Miaomai Zhou,Jason Hill,Abraham Guerrero,Xuewei Wang,Aziz Zaanan,Samuel J Klempner*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: ILUSTRO研究队列4评估了zolbetuximab联合mFOLFOX6化疗及nivolumab一线治疗CLDN18.2阳性转移性胃/胃食管结合部腺癌的疗效与安全性。结果显示中位无进展生存期（PFS）达14.8个月（CLDN18.2高表达者为18.0个月），客观缓解率（ORR）为62.1%，安全性可控，支持在III期LUCERNA研究中进一步验证该联合方案。


<details>
  <summary>Details</summary>
Motivation: 针对CLDN18.2阳性、HER2阴性的转移性胃/胃食管结合部（mG/GEJ）腺癌患者，目前仍缺乏高效且安全的治疗手段。鉴于靶向CLDN18.2和PD-L1的策略具有潜力，且此前Zolbetuximab联合化疗已显示出生存获益，本研究旨在探索Zolbetuximab联合化疗及免疫检查点抑制剂（Nivolumab）的三联疗法作为一线治疗的可行性和初步疗效。

Method: 这是一项全球性、开放标签的II期ILUSTRO研究的队列4（包括4A安全性导入期和4B扩展期）分析报告。研究对象为接受一线Zolbetuximab + mFOLFOX6 + Nivolumab治疗的mG/GEJ腺癌患者。主要疗效终点为由研究者根据RECIST v1.1标准评估的无进展生存期（PFS）。截至2025年9月2日的数据截止点，共纳入77名患者（4A+4B），其中85.5%为CLDN18.2高表达肿瘤。

Result: 在队列4B中（中位随访11.5个月），总体人群（n=71）的中位PFS为14.8个月（95% CI: 8.3-不可估计），CLDN18.2高表达亚组（n=59）的中位PFS延长至18.0个月（95% CI: 11.1-不可估计）。总体人群（n=58）的客观缓解率（ORR）为62.1%（95% CI: 48.4-74.5），CLDN18.2高表达亚组（n=47）的ORR为68.1%（95% CI: 52.9-80.9）。安全性方面，最常见的治疗 emergent 不良事件为恶心（80.5%）和食欲减退（72.7%）。

Conclusion: Zolbetuximab联合化疗和免疫治疗（Nivolumab）在CLDN18.2阳性mG/GEJ腺癌患者中显示出令人鼓舞的疗效和可管理的安全性数据。这些结果支持在正在进行的III期LUCERNA研究中，对CLDN18.2阳性和PD-L1阳性患者进行该联合方案的随机化评估。

Abstract: There is an unmet need for effective and safe treatments for patients with metastatic gastric/gastroesophageal junction (mG/GEJ) adenocarcinoma. Targeting claudin 18 isoform 2 (CLDN18.2) and programmed death ligand 1 (PD-L1), represents a promising strategy. Zolbetuximab, a CLDN18.2-targeting antibody, plus chemotherapy improved survival outcomes in patients with CLDN18.2-positive, human epidermal growth factor receptor 2 (HER2)-negative mG/GEJ adenocarcinoma. Cohort 4 of the global, open-label, phase 2 ILUSTRO study examined first-line zolbetuximab plus mFOLFOX6 and nivolumab (a PD-L1 inhibitor). Here we report results from cohorts 4A (safety lead-in phase) and 4B (expansion phase). The primary endpoint of ILUSTRO was specific to cohort 1 and was previously published; the main efficacy endpoint of interest for cohort 4 was progression-free survival (PFS), as assessed by the investigators per Response Evaluation Criteria in Solid Tumors version 1.1. At data cutoff (2 September 2025) for this final analysis, 77 patients were enrolled in 4A + 4B (85.5% with CLDN18.2-high tumors). Cohort 4B median follow-up was 11.5 months, and median PFS (95% confidence interval (CI)) was 14.8 months (8.3-not estimable) overall (n = 71) and 18.0 months (11.1-not estimable) in patients with CLDN18.2-high tumors (n = 59). Objective response rate (measurable disease; 95% CI) was 62.1% (48.4-74.5) in 4B overall (n = 58) and 68.1% (52.9-80.9) in CLDN18.2-high (n = 47). In 4A + 4B, the most common treatment-emergent adverse events were nausea (80.5%) and decreased appetite (72.7%). Efficacy and safety data support randomized evaluation of zolbetuximab plus chemoimmunotherapy in patients with CLDN18.2-positive and PD-L1-positive mG/GEJ adenocarcinoma in the ongoing phase 3 LUCERNA study. ClinicalTrials.gov: NCT03505320 .

</details>


### [13] [Trastuzumab-Induced Pneumonitis: A Rare but Potentially Severe Pulmonary Complication.](https://pubmed.ncbi.nlm.nih.gov/41994694/)
*Marco Antonio Rodríguez Sánchez,Ana Karen Verdiales Lugo,José Manuel Aguilar Rubio,Carlos Andres Olguin Guizado,Juliana Patricia Ortiz Jiménez,Carlos Orlando López Millán,Lucero Valenzuela Carvajal*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: 本文报告了一例75岁HER2阳性乳腺癌患者在接受曲妥珠单抗治疗后发生罕见但致命的间质性肺炎并进展为急性呼吸窘迫综合征（ARDS）的病例，强调了早期识别、排除感染及及时使用皮质类固醇治疗的重要性。


<details>
  <summary>Details</summary>
Motivation: 尽管曲妥珠单抗通常耐受性良好，但其引起的肺毒性（如间质性肺炎）是一种罕见且可能危及生命的并发症。随着抗HER2疗法应用的扩大，提高对这种严重不良事件的认知至关重要，目前相关病例报告较少，需补充临床证据以指导早期诊断和处理。

Method: 采用病例报告方法，详细描述了一名75岁HER2阳性多形性小叶乳腺癌患者的临床病程。患者接受多西他赛、卡铂、曲妥珠单抗和帕妥珠单抗联合治疗。通过胸部CT影像学检查（双侧磨玻璃影、间质增厚、胸腔积液）、广泛的感染排查（呼吸道病毒面板、支气管培养阴性）以及时间关联性分析，诊断为曲妥珠单抗诱导的肺炎。

Result: 患者在第五个化疗周期后出现急性呼吸衰竭和严重低氧血症，迅速进展为需要侵入性机械通气和俯卧位通气的严重ARDS。虽然高剂量全身皮质类固醇治疗带来了初期的临床和血气改善，但患者随后并发铜绿假单胞菌引起的呼吸机相关性肺炎，最终导致死亡。

Conclusion: 曲妥珠单抗诱导的肺炎是一种罕见但严重的不良反应。临床医生应高度重视其早期识别，迅速排除感染性和心源性病因，并及时启动皮质类固醇治疗以改善预后。随着抗HER2疗法的普及，加强对该并发症的警惕性是保障患者安全的关键。

Abstract: Trastuzumab is a monoclonal antibody targeting the human epidermal growth factor receptor 2 (HER2) and is widely used in the treatment of HER2-positive breast cancer. Although generally well tolerated, trastuzumab has been rarely associated with pulmonary toxicity, including interstitial pneumonitis, a potentially life-threatening complication that may progress to acute respiratory distress syndrome (ARDS). We report the case of a 75-year-old woman with HER2-positive pleomorphic lobular breast carcinoma treated with docetaxel, carboplatin, trastuzumab, and pertuzumab. Shortly after her fifth chemotherapy cycle, she developed acute respiratory failure characterized by severe hypoxemia. Chest computed tomography revealed bilateral ground-glass opacities with interstitial thickening and pleural effusions. Extensive infectious workup, including respiratory viral panels and bronchial cultures, was negative. Given the temporal relationship with trastuzumab exposure and exclusion of alternative diagnoses, trastuzumab-induced pneumonitis was suspected. The patient progressed to severe ARDS requiring invasive mechanical ventilation and prone positioning. High-dose systemic corticosteroid therapy resulted in significant initial clinical and gasometric improvement. However, her course was complicated by ventilator-associated pneumonia due to Pseudomonas aeruginosa, leading to a fatal outcome. This case highlights the importance of early recognition of trastuzumab-induced pneumonitis, prompt exclusion of infectious and cardiogenic causes, and timely initiation of corticosteroid therapy to improve outcomes. Increased awareness of this rare but serious adverse event is essential as the use of anti-HER2 therapies continues to expand.

</details>


### [14] [Real-world efficacy and safety of disitamab vedotin (RC48) plus PD-1 inhibitors in advanced urothelial carcinoma: a single-center retrospective study.](https://pubmed.ncbi.nlm.nih.gov/41971139/)
*Ya-Hui Lv,Xiao-Ran Cui,Lu Yu,Tao Li,Chang Liu,Mei-Cen Liu,Yin-Min Ji,Shu-Yan Xiao,Yi Hu,Tong Zhang,Zhe-Feng Liu*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: 这项单中心回顾性真实世界研究评估了维迪西妥单抗（RC48）联合PD-1抑制剂治疗既往接受过治疗的转移性尿路上皮癌（mUC）患者的疗效与安全性。结果显示，该联合方案具有令人鼓舞的抗肿瘤活性（中位无进展生存期PFS为14.22个月，疾病控制率DCR为96.6%）且安全性可控，但需更大规模的前瞻性研究进一步验证。


<details>
  <summary>Details</summary>
Motivation: 尿路上皮癌（UC）在难治性或转移性阶段预后较差。尽管铂类化疗和PD-1抑制剂改善了一部分患者的结局，但获益有限。鉴于HER2靶向抗体偶联药物维迪西妥单抗（RC48）联合PD-1抑制剂在晚期UC中显示出有希望的抗肿瘤活性，本研究旨在利用真实世界数据评估该联合方案在既往接受过治疗的UC患者中的临床疗效和安全性。

Method: 这是一项单中心、回顾性真实世界研究，纳入了2021年10月至2025年4月期间接受RC48联合PD-1抑制剂治疗的29名既往接受过治疗的mUC患者。主要终点为无进展生存期（PFS）和总生存期（OS），次要终点包括客观缓解率（ORR）和疾病控制率（DCR）。疗效依据RECIST 1.1标准评估，治疗相关不良事件（TRAEs）依据CTCAE 5.0标准分级。此外，还根据年龄、ECOG评分、HER2免疫组化状态、转移灶数量及PD-L1表达进行了亚组分析。

Result: 在29名患者中，ORR为34.5%，DCR高达96.6%，仅1例（3.4%）出现疾病进展。中位PFS为14.22个月，OS终点在数据截止时尚未达到。探索性分析显示，年龄≥65岁、ECOG评分为0以及HER2 IHC 3+的患者PFS数值上更长；转移灶≤2个的患者观察到初步的生存差异。安全性方面，82.8%的患者发生TRAEs，主要为周围神经病变（48.28%）、贫血（17.24%）和白细胞减少（10.34%）。≥3级TRAEs发生率为6.9%，无治疗相关死亡，大多数不良事件通过剂量调整或支持治疗即可管理。

Conclusion: 该回顾性真实世界研究提供了初步证据，表明RC48联合PD-1抑制剂在复发或难治性mUC患者中可能提供令人鼓舞的抗肿瘤活性和可控的安全性 profile。然而，由于样本量小、单中心设计及缺乏对照臂等局限性，结果需谨慎解读，未来需要更大规模的前瞻性研究来验证这些发现。

Abstract: BACKGROUND: Urothelial carcinoma (UC) is a common malignancy with poor prognosis in refractory or metastatic stages. While platinum-based chemotherapy and PD-1 inhibitors have improved outcomes for some, the benefits are limited. Disitamab Vedotin (RC48), an HER2-targeted antibody-drug conjugate, combined with PD-1 inhibitors, has shown promising antitumor activity in advanced UC. This study aims to evaluate the clinical efficacy and safety of RC48 combined with PD-1 inhibitors in previously treated UC patients using real-world data.

METHODS: This single-center, retrospective real-world study included 29 patients with mUC who had received prior treatment and were treated with RC48 plus a PD-1 inhibitor between October 2021 and April 2025. Treatment efficacy was assessed according to the Response Evaluation Criteria in Solid Tumours (RECIST 1.1). Primary endpoints were progression-free survival (PFS) and overall survival (OS). Secondary endpoints included objective response rate (ORR) and disease control rate (DCR). Treatment-related adverse events (TRAEs) were recorded and graded according to the Common Terminology Criteria for Adverse Events (CTCAE) version 5.0. Subgroup analyses were conducted based on age, Eastern Cooperative Oncology Group (ECOG) performance status, tumor human epidermal growth factor receptor 2 (HER2) immunohistochemistry (IHC) status, number of metastatic sites, and programmed cell death ligand 1 (PD-L1) expression.

RESULTS: Among 29 included patients, the median age was 67 years, and 69.0% were male. HER2 IHC showed 2+ expression in 51.7% of patients and 3+ expression in 20.7%. The vast majority of patients (93.1%) received ≥4 treatment cycles. The ORR was 34.5%, with a DCR of 96.6%. Disease progression occurred in only 1 patient (3.4%). Median PFS was 14.22 months. In an exploratory preliminary analysis with limited sample size, patients aged ≥65 years, ECOG performance status 0, and HER2 IHC 3+ numerically demonstrated longer PFS. The OS endpoint had not been reached at data cutoff; preliminary survival differences were observed in patients with ≤2 metastatic sites. Regarding safety, TRAEs occurred in 82.8% of patients, primarily peripheral neuropathy (48.28%), anemia (17.24%), and leukopenia (10.34%). Grade ≥3 TRAEs were reported in 6.9% of patients, with no treatment-related deaths recorded. Most adverse events were manageable through dose adjustments or supportive care.

CONCLUSIONS: This retrospective real-world study provides preliminary evidence that RC48 combined with PD-1 inhibitors may offer encouraging antitumor activity and a manageable safety profile in relapsed or refractory mUC, with a median PFS of 14.22 months and a high DCR observed in this cohort. Given the exploratory nature of this analysis and the study's limitations, including the small sample size, single-center design, and lack of a control arm, these findings should be interpreted with caution. Larger prospective studies are warranted to validate these results.

</details>


### [15] [Radiomics and deep learning fusion model based on multiphasic CT for predicting HER2 expression status in bladder urothelial carcinoma: a multicenter study.](https://pubmed.ncbi.nlm.nih.gov/41971125/)
*Wenbang Pan,Liu Yang,Lin Yang,Chaohui Gu,Qiang Lv,Yingyi Zhang,Mingyue Li*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: 本研究开发了一种基于三相CT的多模态融合模型，用于无创预测膀胱尿路上皮癌（BLCA）患者的HER2表达状态。该模型结合影像组学、深度学习（2.5D ResNet50优于3D）及临床特征，在外部验证集中AUC达0.873，显著优于单一模态模型，为抗体药物偶联物（ADCs）治疗的患者筛选提供了可靠的非侵入性工具。


<details>
  <summary>Details</summary>
Motivation: HER2过表达是膀胱尿路上皮癌中抗体药物偶联物（如RC48）的关键治疗靶点，但传统的免疫组化（IHC）评估受限于肿瘤内异质性和采样偏差。因此，亟需一种无创且可靠的影像学方法来准确预测HER2表达状态，以优化患者筛选。

Method: 纳入来自三家机构的411名接受三相增强CT和HER2 IHC检测的BLCA患者，按治疗标准分为HER2阳性（IHC 2+/3+）和阴性（IHC 0/1+）。数据划分为训练集、内部验证集和外部验证集。研究构建了影像组学模型、2.5D和3D ResNet50深度学习模型、临床模型以及多模态融合模型。通过ROC曲线、校准曲线和决策曲线分析评估模型性能。

Result: 肿瘤架构（无蒂模式）、肾积水、盆腔疼痛和放射性淋巴结状态是HER2阳性的独立预测因子。2.5D ResNet50模型（外部AUC=0.827）显著优于3D模型（AUC=0.608）。多模态融合模型表现最佳，在训练集、内部验证集和外部验证集中的AUC分别为0.960、0.908和0.873。在外部队列中，其准确率、敏感性和特异性分别为0.819、0.857和0.784，显著优于所有单模态模型。

Conclusion: 基于三相CT的多模态模型能够实现对BLCA患者HER2表达状态的无创评估，作为一种有前景的工具，有助于筛选可能从ADC治疗中受益的患者群体。

Abstract: BACKGROUND: Human epidermal growth factor receptor 2 (HER2) overexpression is a key therapeutic target for novel antibody-drug conjugates (ADCs) like disitamab vedotin (RC48) in bladder urothelial carcinoma (BLCA), but immunohistochemistry-based assessment is limited by intratumoral heterogeneity and sampling bias. A noninvasive and reliable imaging-based approach is therefore urgently needed. Therefore, this study aimed to construct a noninvasive, imaging-based multimodal model for predicting HER2 expression status in BLCA using tri-phasic CT.

METHODS: A total of 411 patients from three institutions (2021-2024) who underwent tri-phasic contrast-enhanced computed tomography (CT) and HER2 immunohistochemistry (IHC) were included. Patients were classified as HER2-positive (IHC 2+/3+) or negative (IHC 0/1+) based on therapeutic criteria for ADCs. Patients were divided into training, internal validation, and external validation cohorts. Radiomics models, 2.5D and 3D ResNet50 deep learning models, a clinical model, and a multimodal fusion model were developed. Model performance was evaluated using receiver operating characteristic (ROC) analysis, calibration curves, and decision curve analysis.

RESULTS: Tumor architecture (sessile pattern), hydronephrosis, pelvic pain, and radiological lymph node status were identified as independent predictors of HER2 positivity. The 2.5D ResNet50 model achieved an external area under the curve (AUC) of 0.827, significantly outperforming the 3D model (AUC =0.608). The multimodal fusion model showed the best performance, with AUCs of 0.960, 0.908, and 0.873 in the training, internal validation, and external validation cohorts, respectively. In the external cohort, accuracy, sensitivity, and specificity were 0.819, 0.857, and 0.784. The multimodal model significantly outperformed all single-modality models.

CONCLUSIONS: A tri-phasic CT-based multimodal model enables noninvasive assessment of HER2 expression status in BLCA, providing a promising tool for selection of patients likely to benefit from ADC therapy.

</details>


### [16] [Antigen affinity chromatography for functional separation and efficient characterization of critical monoclonal antibody variants.](https://pubmed.ncbi.nlm.nih.gov/41650542/)
*Julia Baumeister,Miriam Erhardt,Johannes G Wilhelm,Kevin Lam,Sybille Ebert,Frank Rosenau,Michaela Blech,Marco Dehling,Fabian Higel*

Source: PubMed

Main category: EGFR Antibody Therapeutics

TL;DR: 本文开发了一种基于分析型抗原亲和色谱的模块化工作流程，用于根据抗原亲和力功能分离单克隆抗体（mAb）变体。通过优化该流程（以曲妥珠单抗为例），成功鉴定了导致结合力丧失的关键翻译后修饰（如HC-D102异构化和Asu形成），并在另外两种mAb中验证了该方法的适应性，为mAb开发中的风险控制和质量表征提供了有力工具。


<details>
  <summary>Details</summary>
Motivation: 单克隆抗体在生产和储存过程中易发生翻译后修饰，特别是互补决定区（CDR）内的修饰会损害抗原相互作用，被视为关键质量属性。因此，需要一种有效的方法来功能性地分离这些变体并鉴定导致结合力丧失的具体修饰，以支持产品质量控制和早期风险缓解。

Method: 开发了一种使用分析型抗原亲和色谱的模块化工作流程，通过线性pH梯度（pH 7.4 - 2.5）根据抗原亲和力分离mAb变体。以曲妥珠单抗及其抗原Her2优化流程，收集结合缺陷组分，并利用表面等离子体共振（SPR）、尺寸排阻色谱（SEC）、离子交换色谱（IEC）、还原毛细管凝胶电泳（CGE）和肽图分析进行表征。随后将该方法应用于另外两种单克隆抗体以验证其通用性。

Result: 在曲妥珠单抗中，鉴定出HC-D102异构化和LCN30琥珀酰亚胺（Asu）形成为导致结合力丧失的关键修饰，其中HC-D102异构化影响最显著，缺陷组分结合力降低高达6%。在mAb1中，HCCDR异构化和Asu形成导致结合活性降低10%。在mAb2中，分辨出结合活性从2%到66%不等的不同变体，其中HCCDR和LCCDR中的Asu形成是导致结合力丧失的主要原因，且在生理条件下不可逆。

Conclusion: 该分析方法能够针对性地鉴定导致单克隆抗体结合力丧失的关键修饰，具有良好的适应性和通用性。它支持药物开发早期的风险缓解和控制策略制定，并确保对mAb变体进行全面的功能和结构表征，从而保障产品质量。

Abstract: Monoclonal antibodies (mAbs) are prone to post-translational modifications during manufacturing and storage, which can affect product quality. Modifications within the complementarity-determining regions (CDRs) impairing antigen interaction are considered as critical quality attributes. A modular workflow was developed using analytical antigen affinity chromatography to enable functional separation of mAb variants based on their antigen affinity. The workflow was optimized using trastuzumab and its antigen, human epidermal growth factor receptor-2 (Her2). A linear pH gradient (pH 7.4 - 2.5) was employed to separate binding-deficient variants, which were fractionated and characterized using surface plasmon resonance (SPR), size exclusion chromatography (SEC), ion exchange chromatography (IEC), reducing capillary gel electrophoresis (CGE), and peptide mapping. Critical modifications, such as HC-D102 isomerization and LCN30 succinimide (Asu) formation, were identified as contributors to binding loss, with HC-D102 isomerization showing the most significant impact. Binding was reduced by up to 6 % in deficient fractions. The workflow was applied to two additional mAbs, demonstrating its adaptability. For mAb1 variants, binding activity decreased 10 % with HCCDR isomerization and Asu formation being critical, while for mAb2, distinct variants with binding activity ranging from 2 % to 66 % were resolved. In mAb2, Asu formation in HCCDR and LCCDR was identified as the primary modification contributing to binding loss and was found to be irreversible under physiological-like conditions (37 °C, pH 7.4). This approach enables the targeted identification of critical modifications, supports early risk mitigation as well as control strategies during development, and ensures comprehensive characterization of mAb variants.

</details>


<div id='CEACAM5 Cancer'></div>

# CEACAM5 Cancer [[Back]](#toc)

### [17] [A dynamic proteomic signature for the prediction of lung Cancer: a longitudinal analysis in the UK Biobank cohort.](https://pubmed.ncbi.nlm.nih.gov/42068891/)
*Jie Liang,Yi Feng,Daying Gui,Xiangyuan Zheng,Rui Gao,Jingdong Zhou,Jiekai Liang,Jingwen Diao,Juan He,Yilin Zhao,Jianxing He,Xuan Wu,Bo Cheng,Wenhua Liang*

Source: PubMed

Main category: CEACAM5 Cancer

TL;DR: 本研究利用UK Biobank队列数据，分析了37,759名参与者的2,921种血浆蛋白，揭示了肺癌诊断前蛋白质组的动态演变规律。研究识别出340种与风险相关的蛋白质，将其分为长期风险（如CEACAM5，反映细胞粘附失调）和近期风险（如IL6，反映炎症激增）两类轨迹。基于此开发的28蛋白预测签名结合临床因素和多基因风险评分，实现了优异的预测性能（AUC=0.830），为肺癌的早期精准筛查提供了新工具。


<details>
  <summary>Details</summary>
Motivation: 旨在解析肺癌分子机制尚不明确的诊断前阶段，通过绘制血浆蛋白质组的时间演变图谱，获取新的生物学见解并改善肺癌的风险预测能力。

Method: 依托UK Biobank前瞻性队列，对37,759名参与者（含342例新发肺癌病例，中位随访11.7年）的2,921种血浆蛋白进行分析。采用时间分层Cox模型、局部加权散点平滑（LOESS）轨迹建模和层次聚类来表征相对于诊断时间的蛋白质动态变化。使用多算法机器学习流水线开发预测签名，并通过双样本孟德尔随机化推断因果关系。

Result: 1. 鉴定出340种具有显著时间异质性的风险相关蛋白，解析为四种不同的轨迹模式，构建了致癌作用的分子时间线。
2. 长期风险（诊断前>5年）与CEACAM5等蛋白相关，提示早期细胞粘附失调；近期风险（诊断前<5年）以IL6等炎症蛋白激增为特征。
3. 衍生的28蛋白签名结合临床因素和多基因风险评分（PRS），预测性能出色（AUC = 0.830）。
4. 孟德尔随机化分析提示部分风险相关蛋白在肺癌发展中起因果作用。

Conclusion: 研究证实肺癌通过一系列动态的蛋白质变化演变，建立了理解诊断前疾病的新模型。所开发的28蛋白签名是识别活动性疾病进展个体的有力工具，有助于实现肺癌的精准筛查。

Abstract: BACKGROUND: To understand the molecularly obscure pre-diagnostic phase of lung cancer, we mapped the temporal evolution of the plasma proteome for new biological insights and improved risk prediction.

METHODS: Leveraging the UK Biobank prospective cohort, we analyzed 2,921 plasma proteins from 37,759 participants, including 342 incident lung cancer cases identified over a median follow-up of 11.7 years. We employed time-stratified Cox models, locally weighted scatterplot smoothing (LOESS) trajectory modeling, and hierarchical clustering to characterize protein dynamics relative to the time of diagnosis. A multi-algorithm machine learning pipeline was used to develop a predictive signature, and two-sample Mendelian randomization was performed to infer causal relationships.

RESULTS: We identified 340 risk-associated proteins showing significant temporal heterogeneity. Long-term risk (>5 years pre-diagnosis) was linked to proteins like CEACAM5, indicating early dysregulation of cell adhesion. Imminent risk (<5 years) was marked by a surge in inflammatory proteins like IL6. These dynamics were resolved into four distinct trajectory patterns, creating a molecular timeline of carcinogenesis. A machine learning-derived 28-protein signature, integrated with clinical factors and Polygenic Risk Score (PRS), achieved outstanding predictive performance (AUC = 0.830). Mendelian randomization also suggested a causal role for some proteins of 340 risk-associated proteins in lung cancer development.

CONCLUSION: Our findings establish that lung cancer evolves through a dynamic sequence of protein changes. This provides a new model for understanding pre-diagnostic disease, and our 28-protein signature is a powerful tool for precision screening to identify individuals with active disease progression.

</details>


### [18] [Protection motivation theory-based management after surgery for osteoporotic vertebral fractures: Modulating inflammatory factors and pain mediators, and ameliorating hypercoagulability and bone metabolism.](https://pubmed.ncbi.nlm.nih.gov/42071798/)
*Xiaorong Guo,Xuan Wei,Xiaojuan Zhang,Jing Liu*

Source: PubMed

Main category: CEACAM5 Cancer

TL;DR: 本研究评估了基于保护动机理论（PMT）的管理模型对骨质疏松性椎体骨折（OVF）术后患者的影响。结果显示，与对照组相比，PMT组在抑制炎症、减轻疼痛介质合成、改善高凝状态及调节骨代谢方面表现更优，且深静脉血栓发生率更低，术后1个月的JOA评分和Barthel指数更高，表明PMT管理能显著促进患者康复。


<details>
  <summary>Details</summary>
Motivation: 旨在探究基于保护动机理论（PMT）的管理模型如何影响骨质疏松性椎体骨折（OVF）术后患者的炎症因子、疼痛介质、凝血功能及骨代谢指标，以验证该管理模型在改善患者术后临床结果方面的有效性。

Method: 采用前瞻性对照研究设计，纳入2024年2月至2025年3月期间接受手术的74名PMT组患者和71名对照组患者。在术前及术后第5天采集空腹静脉血，通过酶联免疫吸附试验、放射免疫分析法、硫代巴比妥酸法及自动凝血分析仪分别检测炎症因子、骨代谢标志物、疼痛介质、氧化应激标志物及凝血参数。同时记录围手术期不良反应，并在术前及术后1个月使用日本骨科协会（JOA）评分和Barthel指数（BI）问卷评估康复质量，进行组内及组间对比分析。

Result: 两组患者术后IL-1β、IL-6、TNF-α、hs-CRP、SP、PGE2、MDA、D-D、FIB、P-selectin和CTX水平升高，而IL-10、SOD、BGP和OC水平降低（P < .05）。组间比较显示，PMT组的IL-1β、hs-CRP、SP、PGE2、MDA、D-D、FIB和CTX水平显著低于对照组，而IL-10、血小板、BGP和OC水平显著高于对照组（P < .05）。PMT组围手术期深静脉血栓发生率低于对照组。术后1个月，两组的JOA和BI评分均较术前提高，且PMT组评分显著高于对照组（P < .05）。

Conclusion: OVF术后早期采用基于PMT的管理模型有助于通过抑制全身炎症、限制疼痛介质合成、改善高凝状态及normalize紊乱的骨代谢，从而带来显著的临床改善，提升患者康复质量并降低并发症风险。

Abstract: The purpose of this research was to investigate how the protection motivation theory (PMT)-based management model influences inflammatory factors, pain mediators, coagulation function, and bone metabolism in postsurgical osteoporotic vertebral fracture (OVF) patients. Between February 2024 and March 2025, OVF patients undergoing surgery at our center were recruited for this investigation. Ultimately, 74 patients were assigned to the PMT group, compared with 71 who were designated as controls. For each participant, venous blood sampling under fasting conditions was performed preoperatively and on postoperative day 5. We then measured a comprehensive set of parameters using dedicated techniques: inflammatory factors (interleukin-1 beta [IL-1β], interleukin-6, interleukin-10 [IL-10], tumor necrosis factor-α, high-sensitivity C-reactive protein [hs-CRP]) and bone metabolism markers (C-terminal telopeptide of type I collagen [CTX], bone Gla protein [BGP], osteocalcin [OC]) via enzyme-linked immunosorbent assay, pain mediators (substance P [SP], prostaglandin E2 [PGE2]) via radioimmunoassay, oxidative stress markers (malondialdehyde [MDA], superoxide dismutase) via the thiobarbituric acid method, and coagulation parameters (D-dimer [D-D], fibrinogen [FIB], P-selectin, platelet) via an automatic coagulation analyzer. The resulting data were subjected to both intragroup and intergroup comparative analyses. In addition, the adverse reactions during the perioperative period were counted, and the Japanese Orthopaedic Association (JOA) scores and Barthel index (BI) questionnaires were conducted before and 1 month after the operation to evaluate the rehabilitation quality of the patients. Surgical intervention resulted in an increase in IL-1β, interleukin-6, tumor necrosis factor-α, hs-CRP, SP, PGE2, MDA, D-D, FIB, P-selectin, and CTX, as well as a decrease in IL-10, superoxide dismutase, BGP, and OC in both cohorts (P < .05). According to intergroup analyses, the PMT group exhibited lower IL-1β, hs-CRP, SP, PGE2, MDA, D-D, FIB, and CTX, along with higher IL-10, platelet, BGP, and OC, than controls (P < .05). The incidence of perioperative deep vein thrombosis in the PMT group was lower than that in the control group (P < .05). Although the JOA and BI of the 2 groups were higher than those before operation, the JOA and BI of the PMT group were higher than those of the control group at 1 month after operation (P < .05). Early postoperative adoption of PMT-based management following OVF surgery contributes to significant clinical improvements by suppressing systemic inflammation, limiting pain mediator synthesis, ameliorating hypercoagulability, and normalizing disrupted bone metabolism.

</details>


### [19] [B7-H3 Gene Expression Shapes Prognosis and Therapeutic Opportunities Across Prostate Cancer Patient Groups.](https://pubmed.ncbi.nlm.nih.gov/42012455/)
*Allison Makovec,Ava P Gustafson,Nishant Gandhi,Swati Rampalli,Ali T Arafa,Andrew Elliott,Norm Smith,Martin Felices,Philippa R Kennedy,Eugene Shenderov,Akash Patnaik,Vivek Narayan,Elisabeth I Heath,Nicholas A Zorko,Xiaolei Shi,Emmanuel S Antonarakis,Rana R McKay,Justin Hwang*

Source: PubMed

Main category: CEACAM5 Cancer

TL;DR: 本研究分析了8,157例前列腺癌样本，证实B7-H3在不同疾病阶段、组织部位及种族中广泛表达，支持其作为治疗靶点的可行性。高B7-H3表达与AR相关分子特征及TROP2等表面抗原相关。在原发灶和激素敏感性前列腺癌中，高表达预示不良总生存期；而在转移性肿瘤中则预示较好预后。亚洲/太平洋岛裔转移性患者生存率最低。研究建议利用B7-H3进行双靶点治疗策略开发。


<details>
  <summary>Details</summary>
Motivation: B7-H3 (CD276) 是高风险局部晚期和治疗难治性转移性前列腺癌（PC）中一个有前景的治疗靶点。为了指导治疗开发和策略制定，需要全面评估前列腺肿瘤中B7-H3的表达情况、分子特征及其与总生存期（OS）的关系，并综合考虑组织部位、激素敏感性状态和种族差异的影响。

Method: 研究分析了8,157例具有配对DNA/RNA数据的前列腺癌样本。根据组织部位、自我报告的种族以及疾病状态（激素敏感性HSPC、去势抵抗性CRPC或神经内分泌前列腺癌NEPC）进行标注分析。将B7-H3表达量分为高表达（>75百分位）和低表达（<25百分位）。使用Kaplan-Meier曲线和Cox比例风险模型评估总生存期（OS）。

Result: 1. B7-H3表达广泛存在，但随肿瘤部位、激素敏感性状态和种族变化而异。
2. 高B7-H3表达与AR相关转录因子（HOXB13, FOXA1）、AR相关致病性失调（AR-V7, SPOP, FOXA1, TMPRSS2:ERG融合）以及可操作表面抗原（TROP2, NECTIN-4）一致；与谱系可塑性程序调节因子（EZH2, SOX2, ASCL1）及NEPC相关表面抗原（DLL3, CEACAM5）相关性较弱。
3. 在原发性肿瘤和HSPC中，高B7-H3表达预示不良OS（HR分别为1.342和1.30）；但在转移性肿瘤中，高表达预示有利OS（HR: 0.823）。
4. CRPC和NEPC中OS无显著差异，但种族间OS存在差异，亚洲/太平洋岛裔转移性PC患者生存率最差（HR = 3.72）。

Conclusion: B7-H3在各种前列腺癌环境中的持续表达支持其作为治疗靶点的可行性。其与AR相关分子因子、表面抗原以及细胞疗法或抗体药物偶联物（ADC） investigational targets 的关联，提示了潜在的双靶点治疗策略。

Abstract: PURPOSE: B7-H3 (CD276) represents a promising therapeutic target tested in high-risk localized and treatment-refractory metastatic prostate cancer (PC). To guide therapeutic development and treatment strategies, we examined prostate tumors and evaluated expression, molecular features, and overall survival (OS), accounting for tissue site, hormone-sensitivity status, and race.

EXPERIMENTAL DESIGN: 8,157 PC samples with paired DNA/RNA were analyzed based on annotations by tissue site, self-reported race, and disease state: hormone-sensitive (HSPC), castration-resistant (CRPC), or neuroendocrine PC (NEPC). Expression quartiles were B7-H3-high (>75th percentile) or B7-H3-low (<25th percentile). OS was evaluated using Kaplan-Meier and Cox proportional hazards models.

RESULTS: B7-H3 expression was broadly maintained but varied by tumor site, hormone-sensitivity status, and race. High expression aligned with AR-associated transcription factors (HOXB13, FOXA1), AR-associated pathogenic dysregulations (AR-V7, SPOP, FOXA1, TMPRSS2:ERG fusions), and actionable surface antigens (TROP2, NECTIN-4). Weak correlations were found for lineage-plastic program regulators (EZH2, SOX2, ASCL1) and NEPC-associated surface antigens (DLL3, CEACAM5). High B7-H3 expression in primary tumors and HSPCs portended adverse OS (HR: 1.342, 1.30, CI: 1.19-1.512, 1.15-1.46, q < 0.0001), although favorable in metastatic tumors (HR: 0.823, CI: 0.719-0.942, q = 0.0048). No significant differences in OS were observed among CRPCs and NEPCs, although OS varied by race, with poorest survival in Asian/Pacific Islander metastatic PC patients (HR = 3.72, CI: 1.49-9.29, q = 0.012).

CONCLUSIONS: Maintained B7-H3 expression in various PC settings supports its viability as a target. Associations with AR-related molecular factors, surface antigens, and investigative targets for cell therapy or antibody-drug conjugates (ADC) suggest potential dual-targeting strategies.

</details>


### [20] [Deciphering immune and cellular reprogramming during the progression from inflammatory bowel disease to colorectal cancer using multi-omics single-cell and spatial transcriptomics.](https://pubmed.ncbi.nlm.nih.gov/42001187/)
*Seyed Taleb Hosseini,Kimia AminianToosi,Farkhondeh Nemati,Roya BishehKolaei,Youping Deng*

Source: PubMed

Main category: CEACAM5 Cancer

TL;DR: 本研究通过对1436名IBD和CRC患者的整合转录组学及实验分析，发现CEACAM5（CEA）是连接炎症性肠病（IBD）上皮激活与结直肠癌（CRC）肿瘤发生的关键分子标记物，具有区分IBD亚型及诊断CRC的潜在临床价值。


<details>
  <summary>Details</summary>
Motivation: 旨在阐明炎症性肠病（IBD，包括克罗恩病CD和溃疡性结肠炎UC）发病机制背后的免疫异质性和分子机制，特别是探索其与结直肠癌（CRC）之间的潜在联系及生物标志物。

Method: 采用整合高分辨率转录组学分析与实验验证相结合的方法。转录组层面分析了436个人类肠道组织样本（包括415个bulk RNA-seq、18个单细胞RNA-seq和3个空间转录组样本）。同时，对1000名确诊为IBD或CRC的患者进行了血清癌胚抗原（CEA）水平的评估。

Result: 1. 单细胞分析显示患病组织中上皮细胞和干细胞群增加，提示上皮再生增强和免疫激活。
2. 整合分析鉴定出CEACAM5、LGALS1、NUAK1和PDGFRA等疾病相关基因，其中CEACAM5在CD和UC样本中一致上调。
3. 拟时序分析表明CEACAM5在上皮细胞分化后期表达增加，参与黏膜重塑和慢性炎症；空间转录组证实其在大肠癌上皮区域局部过表达。
4. 血清学分析显示CRC患者CEA水平显著高于IBD患者；在IBD内部，CD患者的CEA水平高于UC患者（P < 0.05）。

Conclusion: CEACAM5（CEA）是连接IBD上皮激活与CRC肿瘤发生过程的关键分子标记物，为区分炎症性肠病与结直肠癌以及评估疾病进展提供了潜在的诊断和预后价值。

Abstract: BACKGROUND: Inflammatory bowel disease (IBD) represents a group of chronic and relapsing inflammatory disorders of the gastrointestinal tract, primarily encompassing Crohn’s disease (CD) and ulcerative colitis (UC). To elucidate immune heterogeneity and molecular mechanisms underlying IBD pathogenesis, we performed an integrated high-resolution transcriptomic and experimental analysis across 1436 patients with IBD and CRC.

METHODS: 436 samples in transcriptomics level were analyzed, including 415 bulk RNA-seq, 18 single-cell RNA-seq (10x Genomics), and 3 spatial transcriptomic (Visium) samples from human intestinal tissue. Additionally, serum carcinoembryonic antigen (CEA) levels were evaluated in 1000 patients diagnosed with IBD (CD or UC) and colorectal cancer (CRC).

RESULTS: Single-cell analysis identified major immune and stromal cell types, among which epithelial cells, T cells, B cells, and tissue stem cells were the most abundant in both CD and UC tissues. Comparative profiling revealed an increase in epithelial and stem cell populations in diseased tissues, indicating enhanced epithelial regeneration and immune activation. Integration of bulk and single-cell RNA-seq datasets highlighted several disease-associated genes, including: CEACAM5, LGALS1, NUAK1, and PDGFRA, with CEACAM5 showing consistent upregulation across CD and UC samples. Pseudotime trajectory analysis demonstrated that CEACAM5 expression increased during the later stages of epithelial cell differentiation and suggesting its involvement in mucosal remodeling and chronic inflammation. Spatial transcriptomic mapping confirmed localized CEACAM5 overexpression in epithelial regions of colorectal cancer tissues, further supporting its role in disease progression. Serological analysis revealed that serum CEA levels were significantly higher in CRC compared to IBD and within IBD, Crohn’s disease patients exhibited higher CEA levels than ulcerative colitis patients (P < 0.05).

CONCLUSIONS: Collectively, these findings indicate that CEACAM5 (CEA) serves as a key molecular marker linking epithelial activation in IBD to tumorigenic processes in colorectal cancer and providing potential diagnostic and prognostic value for distinguishing inflammatory bowel disease and colorectal cancer.

SUPPLEMENTARY INFORMATION: The online version contains supplementary material available at 10.1186/s12967-026-08158-2.

</details>


<div id='EGFR Target Biology'></div>

# EGFR Target Biology [[Back]](#toc)

### [21] [Tumor Angiogenesis and EGFR-Mutated Cancers: Structural Insights, Mutation Dynamics, and Innovative Therapeutic Strategies.](https://pubmed.ncbi.nlm.nih.gov/41863464/)
*Altaf Ahmad Shah,Ashutosh Mani,Salman Akhtar*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本文综述了非小细胞肺癌（NSCLC）中EGFR突变及针对各代酪氨酸激酶抑制剂（TKI）的耐药机制，指出现有疗法面临T790M、C797S突变及MET扩增等挑战，并提出结合人工智能、结构导向药物设计及联合疗法的转化研究策略以克服耐药性。


<details>
  <summary>Details</summary>
Motivation: 尽管TKIs显著改善了NSCLC患者的生存率，但耐药机制的出现限制了其长期疗效。亟需系统分析EGFR突变谱、不同代际TKIs的耐药机制及最新药物设计进展，以解决治疗瓶颈。

Method: 通过在主要数据库中进行全面的文献调查，分析EGFR突变特征、与各代TKIs相关的耐药机制，以及旨在克服耐药的近期药物设计进展。

Result: 第一代TKIs（如厄洛替尼）虽有益但必然产生耐药；第二代TKIs（如阿法替尼）对部分耐药路径有效但对关键突变无效；第三代TKIs（如奥希替尼）对T790M突变选择性提高，但受限于新兴的C797S突变和MET扩增。目前正探索联合疗法、变构抑制剂及AI驱动的药物发现等新兴策略。

Conclusion: 深入理解EGFR的结构和突变景观至关重要。整合AI、系统生物学和结构导向药物设计的转化研究有望提高治疗持久性并实现个性化治疗。结合结构洞察与创新治疗策略的聚焦转化方法是实现EGFR驱动癌症长期临床获益的关键。

Abstract: INTRODUCTION: The epidermal growth factor receptor (EGFR) plays a central role in cancer progression, with mutations leading to constitutive activation that drives cell survival, proliferation, and metastasis. Tyrosine kinase inhibitors (TKIs) have significantly improved survival in nonsmall cell lung cancer (NSCLC) patients; however, resistance mechanisms continue to limit their long-term efficacy.

METHODS: A comprehensive literature survey was conducted across major databases to analyze EGFR mutations, resistance mechanisms associated with different generations of TKIs, and recent advances in drug design aimed at overcoming resistance.

RESULTS: First-generation TKIs, such as erlotinib and gefitinib, provide initial clinical benefit but inevitably encounter resistance. Second-generation agents, including afatinib, address some resistance pathways but remain ineffective against critical mutations. Third-generation inhibitors, like osimertinib, demonstrate improved selectivity, particularly against T790M mutations; however, emergent C797S mutations and MET amplifications compromise their effectiveness. Emerging strategies including combination therapies, allosteric inhibitors, and artificial intelligence (AI)- driven drug discovery are actively being explored to overcome these challenges.

DISCUSSION: A deep understanding of the structural and mutational landscape of EGFR is essential for tackling therapeutic resistance in NSCLC. Translational research integrating AI, systems biology, and structure-guided drug design offers promise for improving treatment durability and personalizing therapy. Advances in combination strategies and predictive modeling are reshaping the management of resistant mutations, laying the groundwork for precision oncology in EGFR-mutant cancers.

CONCLUSION: A focused translational approach that combines structural insights with innovative therapeutic strategies is urgently needed to achieve lasting clinical benefits in EGFR-driven cancers.

</details>


### [22] [First-line zolbetuximab plus mFOLFOX6 and nivolumab in unresectable CLDN18.2-positive gastric or gastroesophageal junction adenocarcinoma: a phase 2 trial.](https://pubmed.ncbi.nlm.nih.gov/41840238/)
*Kohei Shitara,Hirokazu Shoji,Nicola Fazio,Sara Lonardi,Keun-Wook Lee,Li-Yuan Bai,Kensei Yamaguchi,Jean-Philippe Metges,Gianluca Masi,Denis Smith,Tae-Yong Kim,Maria Matsangou,Archita Shrivastava,Miaomai Zhou,Jason Hill,Abraham Guerrero,Xuewei Wang,Aziz Zaanan,Samuel J Klempner*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: ILUSTRO研究队列4评估了佐妥昔单抗联合mFOLFOX6化疗及纳武利尤单抗一线治疗转移性胃/胃食管结合部腺癌的疗效与安全性。结果显示，在CLDN18.2高表达患者中，中位无进展生存期达18.0个月，客观缓解率为68.1%，主要不良反应为恶心和食欲减退。数据支持在III期LUCERNA研究中进一步开展随机对照试验。


<details>
  <summary>Details</summary>
Motivation: 针对转移性胃/胃食管结合部（mG/GEJ）腺癌患者，目前仍缺乏高效且安全的治疗方案。鉴于靶向CLDN18.2和PD-L1是具有前景的治疗策略，且此前研究已证实佐妥昔单抗联合化疗可改善CLDN18.2阳性、HER2阴性患者的生存获益，本研究旨在探索佐妥昔单抗联合化疗及免疫检查点抑制剂（纳武利尤单抗）的三联疗法在一线治疗中的潜力。

Method: 采用全球、开放标签、II期ILUSTRO研究的队列4（包括4A安全性导入期和4B扩展期）数据。入组患者接受佐妥昔单抗、mFOLFOX6化疗方案及纳武利尤单抗联合治疗。主要疗效终点为由研究者根据RECIST v1.1标准评估的无进展生存期（PFS）。数据分析截止日为2025年9月2日，共纳入77例患者（4A+4B），其中85.5%为CLDN18.2高表达肿瘤。

Result: 队列4B的中位随访时间为11.5个月。总体人群（n=71）的中位PFS为14.8个月（95% CI: 8.3-不可估计），CLDN18.2高表达亚组（n=59）的中位PFS为18.0个月（95% CI: 11.1-不可估计）。在可测量疾病的队列4B总体人群（n=58）中，客观缓解率（ORR）为62.1%（95% CI: 48.4-74.5），CLDN18.2高表达亚组（n=47）的ORR为68.1%（95% CI: 52.9-80.9）。最常见的治疗 emergent 不良事件为恶心（80.5%）和食欲减退（72.7%）。

Conclusion: 佐妥昔单抗联合化疗和纳武利尤单抗在CLDN18.2阳性及PD-L1阳性mG/GEJ腺癌患者中显示出良好的疗效和可控的安全性。这些结果支持在正在进行的III期LUCERNA研究中对该联合方案进行随机对照评估，以进一步验证其临床价值。

Abstract: There is an unmet need for effective and safe treatments for patients with metastatic gastric/gastroesophageal junction (mG/GEJ) adenocarcinoma. Targeting claudin 18 isoform 2 (CLDN18.2) and programmed death ligand 1 (PD-L1), represents a promising strategy. Zolbetuximab, a CLDN18.2-targeting antibody, plus chemotherapy improved survival outcomes in patients with CLDN18.2-positive, human epidermal growth factor receptor 2 (HER2)-negative mG/GEJ adenocarcinoma. Cohort 4 of the global, open-label, phase 2 ILUSTRO study examined first-line zolbetuximab plus mFOLFOX6 and nivolumab (a PD-L1 inhibitor). Here we report results from cohorts 4A (safety lead-in phase) and 4B (expansion phase). The primary endpoint of ILUSTRO was specific to cohort 1 and was previously published; the main efficacy endpoint of interest for cohort 4 was progression-free survival (PFS), as assessed by the investigators per Response Evaluation Criteria in Solid Tumors version 1.1. At data cutoff (2 September 2025) for this final analysis, 77 patients were enrolled in 4A + 4B (85.5% with CLDN18.2-high tumors). Cohort 4B median follow-up was 11.5 months, and median PFS (95% confidence interval (CI)) was 14.8 months (8.3-not estimable) overall (n = 71) and 18.0 months (11.1-not estimable) in patients with CLDN18.2-high tumors (n = 59). Objective response rate (measurable disease; 95% CI) was 62.1% (48.4-74.5) in 4B overall (n = 58) and 68.1% (52.9-80.9) in CLDN18.2-high (n = 47). In 4A + 4B, the most common treatment-emergent adverse events were nausea (80.5%) and decreased appetite (72.7%). Efficacy and safety data support randomized evaluation of zolbetuximab plus chemoimmunotherapy in patients with CLDN18.2-positive and PD-L1-positive mG/GEJ adenocarcinoma in the ongoing phase 3 LUCERNA study. ClinicalTrials.gov: NCT03505320 .

</details>


### [23] [Hypoxia-Inducible Factor-1α Drives Cellular Senescence via Autophagy Activation to Promote Acute Kidney Injury-to-CKD Transition.](https://pubmed.ncbi.nlm.nih.gov/41838831/)
*Manzhu Zhang,Lingxu Li,Xinyue Mao,Xiaoying Yun,Meiting Wu,Chang Wang,Bing Li*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本研究揭示了HIF-1α在急性肾损伤（AKI）向慢性肾脏病（CKD）转化中的双重作用：早期适度的HIF-1α激活有助于修复，而持续激活则通过自噬介导的细胞衰老促进纤维化和疾病进展。


<details>
  <summary>Details</summary>
Motivation: 尽管已知HIF-1α在AKI早期具有保护作用，且细胞衰老与CKD进展相关，但HIF-1α信号是否介导细胞衰老并推动AKI向CKD转化的机制尚不清楚。

Method: 采用适应性修复（AR）和适应不良修复（MAR）的小鼠缺血再灌注损伤模型，以及人肾近曲小管上皮细胞（HK-2）的缺氧/复氧模型，检测HIF-1α、细胞衰老和自噬的动态变化及相互关系；并通过CKD患者肾活检样本验证HIF-1α水平与细胞衰老的临床相关性。

Result: AR组表现为快速且充足的HIF-1α表达及急性衰老，修复完成后恢复基线；MAR组表现为延迟且持续的HIF-1α激活，驱动慢性衰老和进行性纤维化。体外实验显示，沉默HIF-1α可减弱自噬、衰老和纤维化，而过表达则相反；自噬激动剂雷帕霉素可逆转HIF-1α敲低的抑制作用，自噬抑制剂巴弗洛霉素A1可减弱HIF-1过表达的促衰老和促纤维化效应。临床样本证实HIF-1α升高与eGFR降低、纤维化增加及细胞衰老增强相关。

Conclusion: AKI早期充分的HIF-1α激活与急性衰老及肾脏修复相关，而持续的HIF-1α激活通过持久激活自噬驱动慢性衰老，从而促进AKI向CKD的转化。

Abstract: INTRODUCTION: Ischemia and hypoxia are central drivers of acute kidney injury (AKI), and hypoxia-inducible factor (HIF) is a key regulator of cellular oxygen homeostasis. Our previous study showed that HIF-1α is protective during the early stage of AKI; however, its role in the transition from AKI to chronic kidney disease (CKD) remains unclear. Cellular senescence has been implicated in CKD progression and can be accelerated by hypoxia. Nevertheless, whether HIF-1α signaling mediates cellular senescence and promotes the AKI-to-CKD transition is poorly understood.

METHODS: Ischemia-reperfusion injury models of AKI with adaptive repair (AR) or maladaptive repair (MAR) were used to examine the dynamics of HIF-1α, cellular senescence, and autophagy. In human renal proximal tubular epithelial cells (HK-2), a hypoxia/reoxygenation (H/R) injury model was used to evaluate the relationships among HIF-1α, autophagy, and senescence. Finally, the association between HIF-1α levels and cellular senescence was assessed in renal biopsies from CKD patients.

RESULTS: The AR group exhibited rapid and sufficient HIF-1α expression, accompanied by acute senescence, with HIF-1α levels returning to baseline upon the completion of repair. In contrast, the MAR group showed delayed and sustained HIF-1α activation, driving chronic senescence and progressive fibrosis. The activation of autophagy was closely associated with the expression of HIF-1α in the AR and MAR groups. In vitro, silencing HIF-1α expression attenuated autophagy activity, senescence, and fibrosis in HK-2 cells under H/R. However, HIF-1α overexpression had the opposite effect. The autophagy activator rapamycin reversed the inhibitory effects of HIF-1α knockdown, whereas the autophagy inhibitor bafilomycin A1 diminished the pro-senescent and pro-fibrotic effects of HIF-1α overexpression. Analysis of CKD patient biopsies confirmed elevated HIF-1α expression, which correlated with reduced eGFR, increased fibrosis, and enhanced cellular senescence.

CONCLUSION: Adequate HIF-1α activation during early AKI is associated with acute senescence and renal repair, whereas sustained HIF-1α drives chronic senescence via persistent autophagy activation and may contribute to the AKI-to-CKD transition.

</details>


### [24] [Proenkephalin for Cardiovascular and All-Cause Mortality: The REasons for Geographic and Racial Differences in Stroke Study (REGARDS) Study.](https://pubmed.ncbi.nlm.nih.gov/41838818/)
*Ryan N Pavlovich,Mary Cushman,Suzanne E Judd,Virginia J Howard,Monika M Safford,Alexander L Bullen,Nicholas Wettersten*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本研究评估了新型生物标志物Proenkephalin (PENK) 对全因及心血管死亡率的预测价值。结果显示，在调整了估算肾小球滤过率 (eGFR) 后，PENK 未能提供额外的风险分层信息，表明其作为独立预后指标的价值有限。


<details>
  <summary>Details</summary>
Motivation: 肾功能不全患者面临较高的死亡风险，尤其是心血管死亡风险。然而，现有的肾功能标志物（如 eGFR 和尿白蛋白肌酐比值 UACR）可能无法充分捕捉这一风险。PENK 作为一种反映肾小球功能的新兴生物标志物，其潜在预后价值尚需验证。

Method: 基于 REGARDS 队列研究，随机选取 1,021 名黑人和白人参与者测量 PENK 水平。采用嵌套多变量 Cox 比例风险模型，评估 PENK 与全因及心血管死亡率之间的关联。模型逐步调整混杂因素，包括 UACR 以及使用 2021 CKD-EPI 联合肌酐-胱抑素 C 无种族方程计算的 eGFR，并检验性别和种族的效果修饰作用。

Result: 中位随访 11.6 年间发生 471 例死亡和 142 例心血管死亡。仅调整合并症和 UACR 时，PENK 翻倍与全因死亡风险增加相关 (HR 1.22)，但调整 eGFR后该关联消失 (HR 0.85, p=0.85)。存在显著的性别交互作用 (p=0.004)，女性中较高的 PENK水平与较低的死亡率相关。对于心血管死亡率，调整 eGFR 后 PENK 的显著关联性也消失 (HR 1.36, p=0.20性别或种族间无显著交互作用。

Conclusion: 在当前的生物标志物措施（特别是 eGFR）之外，PENK 并未为全因死亡率和心血管死亡率提供额外的风险分层价值。

Abstract: INTRODUCTION: Individuals with kidney dysfunction have greater risk of mortality, especially cardiovascular mortality, but current markers of kidney function, including estimated glomerular filtration rate (eGFR) and urine albumin-to-creatinine ratio (UACR), may not adequately capture this risk. Proenkephalin (PENK) is an emerging biomarker reflecting kidney glomerular function. We evaluated the association of PENK with all-cause and cardiovascular mortality in The REasons for Geographic and Racial Disparities in Stroke (REGARDS) cohort study and whether associations varied by sex and race.

METHODS: PENK was measured in 1,021 Black and White participants randomly sampled from REGARDS. We evaluated the association of PENK with all-cause and cardiovascular mortality in nested multivariable Cox proportional hazard models adjusting for confounders including UACR and eGFR calculated using the 2021 CKD-EPI combined creatinine-cystatin C race-free equation. Effect modification by sex and race was tested.

RESULTS: Mean age was 67 years, 50% were women, 50% were Black, mean eGFR was 82 mL/min/1.73 m2 and median UACR was 7.7 mg/g. There were 471 deaths and 142 cardiovascular deaths over a median follow-up of 11.6 years. When adjusting for comorbidities and UACR, each doubling of PENK was associated with higher risk of death (HR 1.22, 95% confidence interval [CI] 1.00-1.48, p = 0.05) but this association was not significant after adjusting for eGFR (HR 0.85, 95% CI: 0.65-1.10, p = 0.85). There was a significant interaction by sex (p-interaction = 0.004) with higher PENK levels associated with lower mortality in women. PENK was associated with cardiovascular mortality after adjusting for comorbidities and UACR (HR 1.70, 95% CI: 1.20-2.42, p = 0.004), but this was not significant when adjusting for eGFR (HR 1.36, 95% CI: 0.84-2.21, p = 0.20). There was no significant interaction by sex or race.

CONCLUSION: PENK does not provide additional risk stratification for all-cause and cardiovascular mortality beyond current biomarker measures of eGFR.

</details>


### [25] [Integrative multi-omics analysis reveals an 11-gene malignant-myeloid interaction signature and identifies TPST1 as a potential regulator of immunosuppressive microenvironment in glioma.](https://pubmed.ncbi.nlm.nih.gov/41838327/)
*Junwei Ren,Jie Lu,Zhuwei Zhang,Weikang Xing*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本研究通过整合多组学数据，鉴定出一个包含11个基因的恶性-髓系相互作用特征（MMIS），该特征能准确预测胶质母细胞瘤（GBM）患者的预后及免疫治疗反应。其中，TPST1被确定为关键调控因子，通过特定信号通路促进免疫抑制微环境形成并导致免疫治疗耐药。


<details>
  <summary>Details</summary>
Motivation: 胶质母细胞瘤（GBM）具有高度侵袭性和强烈的免疫抑制肿瘤微环境（TME）。尽管已知髓系细胞（特别是肿瘤相关巨噬细胞）在免疫逃逸中起关键作用，但肿瘤细胞内部调节髓系介导的免疫抑制的具体调控因子尚不清楚，亟需阐明其机制以改善预后评估和治疗策略。

Method: 研究整合了5个胶质瘤单细胞RNA测序数据集、7个带有生存数据的批量RNA测序队列以及7个免疫治疗转录组数据集。通过参考映射建立髓系功能基因集，利用加权基因共表达网络分析（WGCNA）识别髓系相关模块，并与恶性细胞标记物重叠定义出11基因MMIS。通过荟萃分析评估预后和免疫治疗反应，利用去卷积算法和相关性分析评估免疫浸润、免疫抑制标记物及通路活性。

Result: 鉴定的11基因MMIS（包括CLU, MAP1B, IGFBP7等）与髓系功能及不良预后强相关。基于MMIS构建的风险评分优于年龄、IDH状态等传统临床病理因素。该特征与巨噬细胞浸润、免疫抑制标记物（如CD163, TGFB1）及T细胞耗竭特征呈正相关。其中，TPST1在高级别胶质瘤中上调，与免疫治疗耐药相关，并通过PTN-NCL和EREG/AREG-EGFR信号通路与髓系细胞相互作用，促进肿瘤细胞增殖。

Conclusion: 研究验证了一个反映恶性-髓系串扰的11基因特征，能有效预测GBM预后和免疫治疗反应。揭示了胶质瘤细胞调节免疫抑制微环境的新机制，并突出TPST1作为潜在治疗靶点，其与T细胞排斥和不良免疫治疗反应机制相关，但需在胶质瘤特异性队列中进一步验证。

Abstract: BACKGROUND: Glioblastoma (GBM) is a highly aggressive brain tumor with a profoundly immunosuppressive tumor microenvironment (TME). Myeloid cells, especially tumor-associated macrophages, play a key role in immune evasion, yet regulators within tumor cells that shape myeloid-mediated immunosuppression remain poorly characterized.

METHODS: We integrated five single-cell RNA-seq datasets from glioma tissues, seven bulk RNA-seq cohorts with survival data, and seven immunotherapy transcriptomic datasets. Myeloid functional gene sets (microglia, border-associated macrophages, dendritic cells) were established via reference mapping. Weighted gene co-expression network analysis (WGCNA) identified a myeloid-associated module, whose overlap with malignant cell markers defined an 11-gene Malignant–Myeloid Interaction Signature (MMIS). Prognostic and immunotherapy response values were evaluated through meta-analysis. Immune infiltration, immunosuppression markers, and pathway activities were assessed using deconvolution algorithms and correlation analyses.

RESULTS: The 11-gene MMIS (CLU, MAP1B, IGFBP7, NNMT, EMP1, EFEMP1, PAM, TPST1, MT2A, CHI3L1, ACTN1) was strongly correlated with myeloid function and poor prognosis. A risk score based on MMIS was constructed, which outperformed standard clinicopathological factors (including Age, IDH status, 1p/19q codeletion, and MGMT promoter methylation). The signature correlated positively with macrophage infiltration, immunosuppressive markers (e.g., CD163, TGFB1, IL10), and T cell exhaustion signatures (e.g., PDCD1, CTLA4, BATF). Among these, TPST1 was associated with immunotherapy resistance and was upregulated in high-grade glioma. TPST1-high tumor cells exhibited proliferative enrichment and potential interaction with myeloid cells via PTN-NCL and EREG/AREG-EGFR signaling pathways.

CONCLUSIONS: We identified and validated an 11-gene signature that reflects malignant–myeloid crosstalk and predicts prognosis and immunotherapy response in GBM. These findings reveal novel mechanisms through which glioma cells modulate the immunosuppressive microenvironment and highlight TPST1 as a potential therapeutic target associated with T-cell exclusion and poor immunotherapy response mechanisms, though further validation in glioma-specific cohorts is required.

SUPPLEMENTARY INFORMATION: The online version contains supplementary material available at 10.1007/s12672-026-04764-0.

</details>


### [26] [Microbiome and EGFR-mutant non-small cell lung cancer: a complex interplay.](https://pubmed.ncbi.nlm.nih.gov/41820290/)
*Serena Eccher,Marco Sposito,Ilaria Mariangela Scaglione,Luca Pasqualin,Michele Rota,Adele Bonato,Lucia Longo,Alice Avancini,Ilaria Trestini,Daniela Tregnago,Jessica Insolda,Michele Milella,Sara Pilotto,Lorenzo Belluomini*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本文综述了宿主微生物组（肠道、呼吸道及肿瘤内）对EGFR突变非小细胞肺癌（NSCLC）患者接受EGFR-TKI治疗疗效及耐受性的影响，探讨了通过靶向微生物组增强TKI疗效、减轻毒性并潜在改善免疫治疗敏感性的策略。


<details>
  <summary>Details</summary>
Motivation: 尽管EGFR酪氨酸激酶抑制剂（EGFR-TKIs）为EGFR突变NSCLC患者带来显著临床获益，但耐药性的产生和治疗相关毒性仍是主要挑战。新兴证据表明宿主微生物组可能显著影响EGFR靶向治疗的疗效和耐受性，因此需要系统梳理这一领域的研究进展。

Method: 采用文献综述方法，总结EGFR突变NSCLC的主要微生物组特征，讨论肠道、呼吸道和肿瘤内微生物组与EGFR-TKI治疗之间的相互作用，重点分析与不同TKI相关的差异性微生物组变化，并比较微生物组在调节EGFR-TKI反应中的作用。同时，探索旨在增强TKI疗效及提高免疫治疗敏感性的临床前和早期临床策略。

Result: 研究发现微生物组在调节EGFR-TKI疗效和毒性方面发挥关键作用，不同TKI诱导的微生物组shifts存在差异。靶向微生物组的干预措施有望提高TKI疗效，减轻毒性，并可能为分子和免疫学上“冷”的EGFR突变NSCLC亚群扩展免疫治疗选择。

Conclusion: 微生物组研究在EGFR突变NSCLC中具有优化治疗结果的巨大潜力。未来需结合微生物组、代谢组和免疫谱系的整合研究，以将这些见解转化为个性化的临床策略，从而改善患者预后。

Abstract: INTRODUCTION: Epidermal growth factor receptor (EGFR)-mutant non-small cell lung cancer (NSCLC) exhibits unique biological and therapeutic characteristics. Although EGFR tyrosine kinase inhibitors (EGFR-TKIs) offer substantial clinical benefits, resistance development and treatment-related toxicities remain major challenges. Emerging evidence indicates that the host microbiome may significantly influence the efficacy and tolerability of EGFR-targeted therapies.

AREAS COVERED: This review summarizes the main microbiome characteristics of EGFR-mutant NSCLC and discusses the interplay between gut, respiratory and intratumoral microbiome, and EGFR-TKI therapy in NSCLC, highlighting differential microbiome shifts associated with different TKIs and comparing the role of microbiome in modulating responses to EGFR-TKIs. The review also explores preclinical and early clinical strategies aimed at enhancing TKI efficacy and at, potentially, improving sensitivity of EGFR-mutant NSCLC to immunotherapy.

EXPERT OPINION: Despite its emerging role, microbiome research in EGFR-mutant NSCLC holds substantial potential to refine therapeutic outcomes. Microbiota-targeted interventions may improve TKIs efficacy, mitigate toxicity, and potentially expand immunotherapeutic options in this molecularly and immunologically 'cold' subgroup. Future integrative studies combining microbiome, metabolome, and immune profiling are essential to translate these insights into personalized clinical strategies.

</details>


### [27] [EZH2 Inhibitors Sensitize Breast Cancer to HER2 Kinase Inhibitors through Cooperative Effects on YAP and Proapoptotic Regulators.](https://pubmed.ncbi.nlm.nih.gov/41490884/)
*Naiara Perurena,Marina Watanabe,Amy E Schade,William Brown,Elizabeth Y Luo,Alycia Gardner,Alexandra Indeglia,Akiko Yoshinaga,Rhea Sahu,Yoona Yang,Simon R V Knott,Karen Cichowski*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本研究揭示EZH2抑制剂通过表观遗传重编程，协同HER2激酶抑制剂解除YAP/TEAD对促凋亡基因BMF的抑制，从而克服HER2+乳腺癌的耐药性并诱导肿瘤消退。


<details>
  <summary>Details</summary>
Motivation: HER2阳性乳腺癌占所有乳腺癌的20%，尽管已有多种靶向疗法，但原发性或获得性耐药以及转移性疾病仍难以治愈，亟需新的治疗策略来克服耐药性。

Method: 结合体外（in vitro）和体内（in vivo）实验，利用EZH2抑制剂和HER2激酶抑制剂处理HER2+肿瘤模型；通过表观遗传学分析检测H3K27me3水平及YAP/TEAD复合物在BMF基因位点的结合情况；评估联合用药对BMF表达、细胞凋亡及肿瘤生长的影响。

Result: EZH2通过催化H3K27me3沉默促凋亡基因BMF。EZH2抑制剂虽去除了H3K27me3，但导致抑制性YAP/TEAD复合物结合至BMF位点，限制其表达；然而，联合使用HER2激酶抑制剂可触发YAP/TEAD复合物解离，显著上调BMF表达，有效杀死耐药细胞。遗传或药理学抑制YAP/TEAD也能产生类似效果。

Conclusion: EZH2和YAP/TEAD协同隔离BMF位点以维持耐药状态。EZH2抑制剂能够重编程HER2+肿瘤的表观遗传状态，使其对HER2激酶抑制剂高度敏感，二者联用可强效诱导肿瘤消退，为治疗耐药性HER2+乳腺癌提供了极具前景的治疗策略。

Abstract: UNLABELLED: Human epidermal growth factor receptor 2-positive (HER2+) tumors account for 20% of breast cancers. Although a variety of HER2-targeted therapies have been developed, tumors can exhibit de novo or acquired resistance, and metastatic disease remains incurable. In this study, we showed that EZH2 inhibitors shift the epigenetic state of HER2+ tumors, dramatically enhancing baseline responses to HER2 kinase inhibitors and resensitizing drug-resistant tumors in vitro and in vivo. Specifically, EZH2 silenced the proapoptotic gene BMF by catalyzing H3K27 trimethylation (H3K27me3) at regulatory sequences. EZH2 inhibitors promoted the loss of H3K27me3, but this stimulated the binding of repressive YAP/TEAD complexes at the BMF locus, which still restricted expression. However, in the presence of EZH2 inhibitors, HER2 kinase inhibitors triggered the dissociation of repressive YAP/TEAD complexes, potently upregulated BMF, and killed resistant cells. Accordingly, EZH2 inhibitors cooperated with genetic or pharmacologic inhibition of YAP/TEAD, which similarly induced BMF expression and apoptosis. Together, these findings show how EZH2 and YAP/TEAD coordinately insulate the BMF locus and demonstrate that EZH2 inhibitors can be used to reprogram HER2+ tumors, resulting in a dramatic sensitization to HER2 kinase inhibitors and enhanced killing of residual disease.

SIGNIFICANCE: The combination of EZH2 inhibitors and HER2 kinase inhibitors potently induces tumor regression in HER2+ tumors through effects on YAP and proapoptotic proteins, providing a promising therapeutic strategy for breast cancer.

</details>


### [28] [Incidence and Radiological Spectrum of Interstitial Lung Disease in Patients With Lung Cancer Treated With Tyrosine Kinase Inhibitors: A Single-Center Observational Study.](https://pubmed.ncbi.nlm.nih.gov/41988621/)
*Supantha De,Prasanta R Mohapatra,Mahismita Patro,Sudipta Mohakud,Pritinanda Mishra,Sourin Bhuniya,Manoj Panigrahi,Shakti Bal,Mukund Sable,Aneri Parekh,Siddarth Singh,Nilava Banerjee*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 这项单中心观察性研究评估了106名接受酪氨酸激酶抑制剂（TKI）治疗的驱动基因阳性非小细胞肺癌（NSCLC）患者。研究发现，TKI诱导的间质性肺病（ILD）发生率较低（3.7%），所有原因引起的ILD发生率为16.0%。总体疾病控制率为68.8%，平均无进展生存期（PFS）为10.4个月。吸烟状态与治疗反应相关，ECOG评分预测PFS，但无基线因素独立预测ILD。结论认为TKI疗效良好且ILD风险可控，但仍需持续监测。


<details>
  <summary>Details</summary>
Motivation: 尽管TKI改善了驱动基因阳性NSCLC患者的预后，但药物诱导的间质性肺病（ILD）作为一种罕见但具有临床意义的毒性，其具体发生率、放射学特征、相关不良反应、风险因素及对无进展生存期（PFS）的影响仍需明确，以指导临床风险管理。

Method: 采用单中心观察性研究设计，纳入2024年1月至2025年9月在印度东部某三级医院接受TKI治疗的连续突变阳性肺癌患者。收集基线人口统计学、临床、放射学和分子数据。对患者进行为期12个月的随访，包括结构化临床评估、实验室监测和影像学检查。将ILD分类为TKI归因性和全因性，并分析相关风险因素及生存数据。

Result: 在106名患者中，多数为50岁以上（80.2%）、男性（54.7%）、非吸烟者（85.8%）及晚期疾病（93.4%）。主要突变为EGFR exon19（50.9%）和exon21（16.0%），常用药物为厄洛替尼（48.1%）和奥希替尼（22.6%）。TKI诱导的ILD发生率为3.7%，全因ILD为16.0%；95.3%的患者未显示ILD放射学模式。疾病控制率为68.8%，平均PFS为10.4 ± 2.91个月，12个月生存率为61.2%。84.0%的患者在一年时无CTCAE分级毒性。吸烟状态与治疗反应相关（p=0.045），ECOG评分预测PFS（p=0.048），但无基线因素独立预测ILD。

Conclusion: TKI治疗显示出良好的疾病控制率，且归因性ILD发生率较低。尽管严重毒性罕见，但持续的监测和结构化的风险评估对于管理潜在风险至关重要。

Abstract: Background Tyrosine kinase inhibitors (TKIs) have improved outcomes in driver mutation-positive non-small cell lung cancer (NSCLC), yet drug-induced interstitial lung disease (ILD) remains an uncommon but clinically relevant toxicity. Objectives To determine the incidence and radiological spectrum of ILD among lung cancer patients receiving TKIs, describe accompanying adverse effects, identify clinical and treatment-related risk factors, and estimate progression-free survival (PFS). Methods This single-center observational study included consecutive mutation-positive lung cancer patients treated with TKIs between January 2024 and September 2025 at a tertiary institute in Eastern India. Baseline demographic, clinical, radiological, and molecular data were recorded. Patients were followed for 12 months with structured clinical assessment, laboratory monitoring, and imaging. ILD was classified as TKI-attributable or all-cause. Results Among 106 patients, most were aged 50 years or older (80.2%), male participants (54.7%), non-smokers (85.8%), and had advanced-stage disease (93.4%). Epidermal Growth Factor Receptor (EGFR) exon19 (50.9%) and exon21 (16.0%) mutations dominated in the group. Erlotinib (48.1%) and osimertinib (22.6%) were most frequently used. TKI-induced ILD occurred in 3.7% of patients, while all-cause ILD occurred in 16.0%. Radiologically, 95.3% of the study group showed no ILD pattern. Disease control was achieved in 68.8% of cases. Mean PFS was 10.4 ± 2.91 months, with a 12-month survival of 61.2%. Mild toxicities predominated; 84.0% had no Common Terminology Criteria for Adverse Events (CTCAE)-graded toxicity at one year. Smoking status correlated with treatment response (p=0.045), and Eastern Cooperative Oncology Group (ECOG) performance predicted PFS (p=0.048). No baseline factor independently predicted ILD. Conclusions TKIs demonstrated favorable disease control with low attributable ILD. Continued monitoring and structured risk assessment remain essential.

</details>


### [29] [Real-world efficacy and safety of disitamab vedotin (RC48) plus PD-1 inhibitors in advanced urothelial carcinoma: a single-center retrospective study.](https://pubmed.ncbi.nlm.nih.gov/41971139/)
*Ya-Hui Lv,Xiao-Ran Cui,Lu Yu,Tao Li,Chang Liu,Mei-Cen Liu,Yin-Min Ji,Shu-Yan Xiao,Yi Hu,Tong Zhang,Zhe-Feng Liu*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 这项单中心回顾性真实世界研究评估了维迪西妥单抗（RC48）联合PD-1抑制剂治疗既往接受过治疗的晚期尿路上皮癌（mUC）患者的疗效与安全性。结果显示，该联合方案客观缓解率（ORR）为34.5%，疾病控制率（DCR）高达96.6%，中位无进展生存期（PFS）为14.22个月，且安全性可控。尽管样本量小且缺乏对照组，但结果提示该方案具有潜在的临床获益值得进一步前瞻性验证。


<details>
  <summary>Details</summary>
Motivation: 尿路上皮癌（UC）在难治性或转移阶段预后较差。虽然铂类化疗和PD-1抑制剂改善部分患者结局，但受益有限。鉴于HER2靶向抗体偶联药物维迪西妥单抗（RC48）联合PD-1抑制剂在晚期UC中显示出有希望的抗肿瘤活性，本研究旨在利用真实世界数据评估该联合方案在既往治疗过的UC患者中的临床疗效和安全性。

Method: 这是一项单中心、回顾性真实世界研究，纳入2021年10月至2025年4月期间接受RC48联合PD-1抑制剂治疗的29名既往治疗过的mUC患者依据RECIST 1.1标准评估疗效。主要终点为无进展生存期（PFS）和总生存期（OS）；次要终点包括客观缓解率（ORR）和疾病控制率（DCR）。记录并根据CTCAE v5.0分级治疗相关不良事件（TRAEs）。基于年龄、ECOG评分、HER2 IHC状态、转移灶数量及PD-L1表达进行亚组分析。

Result: 在29名患者中，ORR为34.5%，DCR为96.6%，仅1例（3.4%）出现疾病进展。中位PFS为14.22个月，OS尚未达到。初步亚组分析显示，年龄≥65岁、ECOG评分为0及HER2 IHC 3+的患者PFS数值上更长；转移灶≤2个的患者观察到初步生存差异。82.8%的患者发生TRAEs，主要为周围神经病变（48.28%）、贫血（17.24%）和白细胞减少（10.34%）。≥3级TRAEs发生率为6.9%，无治疗相关死亡，多数不良事件可通过剂量调整或支持治疗管理。

Conclusion: 该回顾性真实世界研究提供初步证据，表明RC48联合PD-1抑制剂在复发或难治性mUC中可能提供令人鼓舞的抗肿瘤活性和可控的安全性 profile（中位PFS 14.22个月，高DCR）。鉴于分析的探索性质及研究局限性（小样本、单中心、无对照臂），结果需谨慎解读，需更大规模的前瞻性研究以验证这些结果。

Abstract: BACKGROUND: Urothelial carcinoma (UC) is a common malignancy with poor prognosis in refractory or metastatic stages. While platinum-based chemotherapy and PD-1 inhibitors have improved outcomes for some, the benefits are limited. Disitamab Vedotin (RC48), an HER2-targeted antibody-drug conjugate, combined with PD-1 inhibitors, has shown promising antitumor activity in advanced UC. This study aims to evaluate the clinical efficacy and safety of RC48 combined with PD-1 inhibitors in previously treated UC patients using real-world data.

METHODS: This single-center, retrospective real-world study included 29 patients with mUC who had received prior treatment and were treated with RC48 plus a PD-1 inhibitor between October 2021 and April 2025. Treatment efficacy was assessed according to the Response Evaluation Criteria in Solid Tumours (RECIST 1.1). Primary endpoints were progression-free survival (PFS) and overall survival (OS). Secondary endpoints included objective response rate (ORR) and disease control rate (DCR). Treatment-related adverse events (TRAEs) were recorded and graded according to the Common Terminology Criteria for Adverse Events (CTCAE) version 5.0. Subgroup analyses were conducted based on age, Eastern Cooperative Oncology Group (ECOG) performance status, tumor human epidermal growth factor receptor 2 (HER2) immunohistochemistry (IHC) status, number of metastatic sites, and programmed cell death ligand 1 (PD-L1) expression.

RESULTS: Among 29 included patients, the median age was 67 years, and 69.0% were male. HER2 IHC showed 2+ expression in 51.7% of patients and 3+ expression in 20.7%. The vast majority of patients (93.1%) received ≥4 treatment cycles. The ORR was 34.5%, with a DCR of 96.6%. Disease progression occurred in only 1 patient (3.4%). Median PFS was 14.22 months. In an exploratory preliminary analysis with limited sample size, patients aged ≥65 years, ECOG performance status 0, and HER2 IHC 3+ numerically demonstrated longer PFS. The OS endpoint had not been reached at data cutoff; preliminary survival differences were observed in patients with ≤2 metastatic sites. Regarding safety, TRAEs occurred in 82.8% of patients, primarily peripheral neuropathy (48.28%), anemia (17.24%), and leukopenia (10.34%). Grade ≥3 TRAEs were reported in 6.9% of patients, with no treatment-related deaths recorded. Most adverse events were manageable through dose adjustments or supportive care.

CONCLUSIONS: This retrospective real-world study provides preliminary evidence that RC48 combined with PD-1 inhibitors may offer encouraging antitumor activity and a manageable safety profile in relapsed or refractory mUC, with a median PFS of 14.22 months and a high DCR observed in this cohort. Given the exploratory nature of this analysis and the study's limitations, including the small sample size, single-center design, and lack of a control arm, these findings should be interpreted with caution. Larger prospective studies are warranted to validate these results.

</details>


### [30] [Diabetes Duration Is Associated with Declining Kidney Function: eGFR and CKD Burden Across Duration.](https://pubmed.ncbi.nlm.nih.gov/41899160/)
*Carmen Pantis,Cosmin Mihai Vesa,Timea Claudia Ghitea,Daniela Florina Trifan,Roxana Daniela Brata,Nicolae Ovidiu Pop,Madalina Ioana Moisi*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本研究通过横断面分析证实，2型糖尿病病程是肾功能下降和慢性肾脏病（CKD）风险的独立预测因子。即使校正了年龄、高血压和代谢因素，病程每增加5年，CKD风险显著增加66%；病程≥15年的患者患CKD的风险是病程0-4年患者的近4倍。


<details>
  <summary>Details</summary>
Motivation: 糖尿病肾病是2型糖尿病（T2DM）的主要并发症及全球慢性肾脏病（CKD）的主要原因。尽管糖尿病病程常被视为累积代谢暴露的标志，但其独立于衰老和高血压之外对肾脏衰退的具体贡献尚未完全明确。本研究旨在量化糖尿病病程对肾功能的独立影响。

Method: 研究纳入250名T2DM成年患者进行横断面研究。将糖尿病病程作为连续变量及四个分层（0-4, 5-9, 10-14, ≥15年）进行分析。主要终点为估算肾小球滤过率（eGFR）。构建未调整及调整后（年龄、性别、BMI、高血压、HbA1c）的线性回归和逻辑回归模型，并进行每5年病程增量的敏感性分析。

Result: 1. eGFR随病程分层显著下降（p < 0.001），病程≥15年组的功能性CKD患病率高达54.2%，显著高于短病程组（~15-18%）。
2. 线性回归显示，病程每增加1年，eGFR独立下降0.85 mL/min/1.73 m²（p < 0.001）。
3. 逻辑回归显示，病程每增加1年，调整后CKD odds增加10.7%（OR=1.11）；每增加5年，CKD风险增加66%（OR=1.66）。
4. 与0-4年病程组相比，≥15年病程组的调整后CKD odds高出近4倍（OR=3.90）。

Conclusion: 糖尿病病程与肾功能下降呈强烈且独立的关联。长期的疾病暴露显著增加CKD风险，这种影响独立于年龄、高血压及代谢因素。结果强调了累积高血糖暴露的进行性肾毒性作用，突显了在T2DM管理中早期并持续采取肾脏保护策略的重要性。

Abstract: Background: Diabetic kidney disease is a major complication of type 2 diabetes mellitus (T2DM) and a leading cause of chronic kidney disease (CKD) worldwide. While diabetes duration is often considered a marker of cumulative metabolic exposure, its independent contribution to renal decline beyond aging and hypertension remains incompletely defined. Methods: We conducted a cross-sectional study including 250 adults with T2DM. Diabetes duration was analyzed both as a continuous variable and across four predefined strata (0-4, 5-9, 10-14, and ≥15 years). The primary endpoint was estimated glomerular filtration rate (eGFR), analyzed as a continuous outcome. Functional CKD was defined as eGFR < 60 mL/min/1.73 m2. Linear and logistic regression models were constructed in unadjusted and adjusted forms (age, sex, BMI, hypertension, HbA1c). A sensitivity analysis modeling duration per 5-year increase was performed. Results: Mean eGFR declined significantly across duration strata (82.45, 84.27, 78.72, and 61.57 mL/min/1.73 m2, respectively; p < 0.001). The prevalence of functional CKD increased markedly in patients with ≥15 years of diabetes (54.2%) compared with shorter-duration groups (~15-18%; p < 0.001). In linear regression, each additional year of diabetes was associated with a 1.32 mL/min/1.73 m2 decline in eGFR (p < 0.001), remaining significant after adjustment (β = -0.85; p < 0.001). In logistic regression, each additional year was associated with a 10.7% increase in adjusted odds of CKD (OR = 1.11; 95% CI 1.04-1.17; p < 0.001). Each 5-year increment conferred a 66% increase in adjusted CKD risk (OR = 1.66; 95% CI 1.25-2.21; p < 0.001). Patients with ≥15 years of diabetes had nearly fourfold higher adjusted odds of CKD compared with those with 0-4 years (OR = 3.90; 95% CI 1.42-10.75; p = 0.008). Conclusions: Diabetes duration is strongly and independently associated with declining kidney function. Prolonged disease exposure confers a substantial increase in CKD risk, even after adjustment for age, hypertension, and metabolic factors. These findings highlight the progressive nephrotoxic impact of cumulative hyperglycemic exposure and underscore the need for early and sustained nephroprotective strategies in T2DM.

</details>


### [31] [Early-Onset Oral Squamous Cell Carcinoma: Emerging Biological Insights, Risk Factors and Clinical Implications.](https://pubmed.ncbi.nlm.nih.gov/41833405/)
*Gennaro Musella,Cristina D'Antonio,Federica Canfora,Michele D Mignogna,Valentino Vellone,Vito Carlo Alberto Caponio,Amerigo Giudice,Alessandro Villa,Daniela Adamo*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本综述通过证据映射和叙事综合，总结了早发性口腔鳞状细胞癌（EO-OSCC）在流行病学、风险因素、分子特征及临床管理方面的现状。研究发现EO-OSCC具有独特的生物学特征（如低肿瘤突变负荷、特定基因改变及免疫抑制微环境），且与含糖饮料、电子烟等新兴风险因素相关。尽管病理特征具侵袭性年轻患者对强化治疗耐受性较好。结论强调亟需开发年龄特异性的预后工具、生物标志物指导的治疗及早期检测方案。


<details>
  <summary>Details</summary>
Motivation: 早发性口腔鳞状细胞癌（EO-OSCC，定义为50岁以下发病）日益被视为一个具有独特暴露模式和肿瘤生物学的独立临床亚群。与传统OSCC强烈关联烟草和酒精不同，EO-OSCC更常影响从不吸烟/饮酒者，这给早期识别、风险分层和管理带来了挑战，因此需要系统梳理现有知识以填补认知空白。

Method: 采用证据映射（evidence mapping）结合叙事综合（narrative synthesis）的方法。文献检索涵盖PubMed、Scopus、Embase、Web of Science和Cochrane Library数据库（截至2025年8月），并补充全球癌症数据库（GLOBOCAN/Cancer Tomorrow）和国际试验注册中心的数据。

Result: 1. **生物学特征**：表现为较低的肿瘤突变负荷，CDKN2A、TP53、NOTCH1和EGFR的复发性改变，表观遗传失调，以及以检查点为主导的免疫抑制性肿瘤微环境。
2. **风险因素**：除传统致癌物外，含糖饮料、电子烟使用和口腔微生物失调成为潜在的可变决定因素，但证据存在异质性。
3. **临床结果**：尽管组织病理学特征常显示侵袭性，但年轻患者对强化多模式治疗的耐受性更好，可能获得良好的预后。

Conclusion: 研究结果突显了针对EO-OSCC患者群体开发年龄特异性预后工具、生物标志物指导疗法及早期检测协议的紧迫性。国际协作对于提高这一增长中患者群体的生存率和长期生活质量至关重要。

Abstract: BACKGROUND: Early-onset oral squamous cell carcinoma (EO-OSCC), commonly defined as occurring in individuals under 50 years of age, is increasingly recognized as a potentially distinct clinical subset with differences in exposure patterns and tumor biology compared with conventional oral squamous cell carcinoma (OSCC). Unlike typical OSCC, which is strongly associated with tobacco and alcohol, EO-OSCC more frequently affects never-smokers/never-drinkers, posing challenges for early recognition, risk stratification, and management.

OBJECTIVE: This review integrates evidence mapping with narrative synthesis to summarize current knowledge on EO-OSCC, focusing on epidemiology, emerging risk factors, molecular alterations, tumor microenvironment characteristics, and clinical implications.

METHODS: Literature was searched across PubMed, Scopus, Embase, Web of Science, and the Cochrane Library up to August 2025, complemented by global cancer databases (GLOBOCAN/Cancer Tomorrow) and international trial registries.

RESULTS: Available data suggest distinct biological features, including relatively low tumor mutational burden, recurrent alterations in CDKN2A, TP53, NOTCH1, and EGFR, epigenetic dysregulation, and a checkpoint-dominant immunosuppressive tumor microenvironment. Alongside traditional carcinogens, modifiable determinants such as sugar-sweetened beverages, electronic cigarette use, and oral microbial dysbiosis have emerged as plausible contributors, although evidence remains heterogeneous. Despite frequently aggressive histopathologic features, younger patients may tolerate intensive multimodal therapies better and achieve favorable outcomes.

CONCLUSION: These findings highlight the urgent need for age-specific prognostic tools, biomarker-guided therapies, and early detection protocols. International collaboration will be key to improving survival and long-term quality of life in this growing patient population.

</details>


### [32] [Glycolytic metabolic reprogramming and mitochondrial dysfunction: A novel mechanism underlying PAEs-promoted cartilage inflammatory phenotype.](https://pubmed.ncbi.nlm.nih.gov/41833247/)
*Lu Liu,Yongchang Chen,Wenying Lin,Mian Zheng,Tianqi Yu,Ruyan Chen,Yanru Yang,Lingling Jiao,Qinli Ruan,Jia Sun*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本研究结合网络毒理学、分子对接及实验验证，揭示了邻苯二甲酸酯（PAEs）通过激活CASP3通路诱导软骨细胞凋亡，并通过重编程糖酵解代谢、抑制氧化磷酸化及破坏线粒体稳态，促进炎症表型形成，从而加剧膝骨关节炎中的软骨损伤。


<details>
  <summary>Details</summary>
Motivation: 尽管邻苯二甲酸酯（PAEs）作为广泛存在的环境内分泌干扰物已知与多种疾病发病机制相关，但其导致骨相关疾病中软骨损伤的具体作用及内在分子机制尚不明确，亟需阐明其与膝骨关节炎（KOA）核心病理变化（关节软骨进行性退化）之间的关联。

Method: 1. **计算预测**：采用网络毒理学结合分子对接技术，系统探索PAEs与KOA的关联，筛选核心调控分子（MMP9, EGFR, IL-10, BCL2, CASP3）。
2. **实验验证**：对人关节软骨细胞（HCs）进行暴露实验，验证核心分子的作用。
3. **机制探究**：通过蛋白质组学分析和细胞实验，深入解析PAEs暴露下的代谢重编程及线粒体功能变化。

Result: 1. 筛选出MMP9、EGFR、IL-10、BCL2和CASP3为核心调控分子。
2. PAEs暴露激活CASP3通路，诱导人软骨细胞凋亡，并显著促进软骨细胞炎症表型的形成。
3. 机制上，PAEs触发糖酵解代谢重编程，抑制氧化磷酸化，并破坏线粒体稳态。

Conclusion: 该研究为理解PAEs暴露驱动的软骨细胞炎症表型及损伤机制提供了坚实的理论基础，并为针对PAEs的新型治疗干预设计及环境毒性评估协议的优化奠定了基础。

Abstract: Phthalate esters (PAEs), the most prevalent class of plasticizers, are widely acknowledged as environmental endocrine disruptors (EDCs) and have been linked to the pathogenesis of various diseases. However, their specific role and intrinsic mechanism in inducing cartilage damage in bone-related diseases remain unclear. This study aimed to clarify the correlation between PAEs and cartilage damage, and to explore their potential molecular mechanisms. The core pathological change of knee osteoarthritis (KOA) is the progressive degeneration of articular cartilage. Based on this, this study first systematically explored the association between PAEs and KOA using network toxicology combined with molecular docking technology, and screened out core regulatory molecules including MMP9, EGFR, IL-10, BCL2, and CASP3. Subsequently, verification experiments on core molecules showed that PAEs exposure could activate the CASP3 pathway to induce apoptosis of human chondrocytes and significantly promote the formation of the inflammatory phenotype of chondrocytes. To further reveal the underlying mechanism, we conducted proteomic analysis and cell experiments in human articular chondrocytes (HCs). The results showed that exposure to PAEs triggered reprogramming of glycolysis metabolism, inhibited oxidative phosphorylation, and damaged mitochondrial homeostasis. Collectively, this study provides a robust theoretical basis for understanding the inflammatory phenotypes and underlying mechanisms of chondrocyte damage driven by PAEs exposure, and further lays a foundation for the design of novel therapeutic interventions and the optimization of environmental toxicity assessment protocols targeting PAEs.

</details>


### [33] [Nitenpyram promotes breast cancer progression via PARP1: Integration of multi-omics analyses with experimental validation.](https://pubmed.ncbi.nlm.nih.gov/41793917/)
*Dongliang Yin,Haoyu Qin,Menglan Yan,Mengyan Lai,Xianhuan Zhou,Shiqi Li,Jiaqiang Wu,Kangping Yang,Liang Yang*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本研究通过多平台生物信息学分析、分子对接及实验验证，揭示了杀虫剂吡虫啉（Nitenpyram, NIT）通过与KRAS、PARP1等关键蛋白相互作用，破坏免疫平衡和细胞稳态，从而促进乳腺癌细胞增殖的致癌机制。


<details>
  <summary>Details</summary>
Motivation: 尽管已有研究提示杀虫剂NIT可能与癌症发展有关，但其具体的致癌机制尚不明确，亟需深入探究其分子层面的作用机理。

Method: 整合多平台生物信息学分析、蛋白质-蛋白质相互作用（PPI）网络构建、基因富集分析及分子对接技术；利用机器学习模型定量分析26个核心基因对多种恶性肿瘤预后的影响权重；结合单细胞转录组数据分析基因在免疫细胞中的表达；最后通过蛋白质定量分析和细胞克隆形成实验进行体外验证。

Result: 分子对接显示NIT与KRAS、PARP1、KIT、ITGB1、EGFR和CXCR4六种蛋白存在潜在相互作用（结合能<-6 kcal/mol）；单细胞数据表明这些基因主要表达于T细胞、B细胞等免疫细胞亚群；实验证实NIT暴露促进乳腺癌细胞增殖，并伴随PARP1表达增加，且短期高剂量暴露效应更显著。

Conclusion: NIT通过干扰关键信号分子（如PARP1等），导致遗传稳定性降低、免疫失衡及细胞稳态破坏，为癌症发展提供有利条件，特别是短期高剂量暴露会显著促进癌细胞增殖。

Abstract: Nitenpyram (NIT) is an insecticide used primarily for flea control in pets, especially cats and dogs. Some studies suggest that NIT is associated with cancer development, but the mechanisms involved are unclear. In this study, we integrated multi-platform bioinformatics analysis, protein-protein interaction (PPI) network construction, gene enrichment analysis and molecular docking to explore the carcinogenic mechanism of NIT in depth. We first targeted the oncogenic targets under NIT exposure, and then mapped the cross-cancer expression profiles based on 26 core genes, and quantitatively analyzed the impact weights of these genes on the prognosis of multiple malignant tumors with the help of machine learning models. Molecular docking suggested potential interactions between NIT and six proteins (KRAS, PARP1, KIT, ITGB1, EGFR, and CXCR4), with predicted binding energies below -6 kcal/mol. Single-cell transcriptomic data indicated that these genes are mainly expressed in T cells, B cells, and other immune cell subsets. Disruption of these molecules by NIT may result in reduced genetic stability, immune imbalance and disruption of cellular homeostasis, providing favorable conditions for cancer development. Finally, through integrating protein quantification analysis and cell colony formation assays, we confirmed that exposure to NIT promotes the proliferation of breast cancer cells, concomitant with an increase in PARP1 expression. Notably, short-term exposure to high doses of NIT exerts a more pronounced effect on cell proliferation.

</details>


### [34] [Exosome biosensors for detection of Lung cancer.](https://pubmed.ncbi.nlm.nih.gov/41565091/)
*Obaid Afzal,Pavan Goud,Kavita Goyal,Ali Altharawi,Mubarak A Alamri,Manal A Alossaimi,Abdulmalik S A Altamimi,Surya Nath Pandey*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本文综述了基于外泌体的生物传感器在肺癌诊断中的应用，涵盖了电化学、光学、微流控及纳米材料辅助等多种检测策略，并探讨了外泌体在治疗递送中的优势。文章指出了当前面临的外泌体异质性、分离标准化缺乏等挑战，并展望了结合人工智能和便携式设备的临床转化前景，旨在推动肺癌的早期筛查、治疗监测及复发检测。


<details>
  <summary>Details</summary>
Motivation: 肺癌是全球癌症死亡的主要原因，常因晚期确诊及肿瘤的高度异质性导致治疗效果不佳。传统的影像学和组织活检具有侵入性且难以反映肿瘤的动态生物学特征。因此，亟需开发非侵入性、能实时反映肿瘤状态的液体活检技术，而富含肿瘤特异性标志物（如蛋白质、脂质、RNA）的外泌体成为了理想的生物标志物来源。

Method: 本文采用综述方法，系统总结了用于检测肺癌外泌体的各类生物传感器策略，包括电化学、光学、微流控和纳米材料辅助技术，重点分析其对PD-L1、EGFR及致癌miRNAs/lncRNAs等标志物的检测能力。同时，对比了外泌体与合成纳米载体（如脂质体、聚合物纳米粒子）在生物相容性、 cargo 保护及靶向能力上的差异，并讨论了外泌体在药物/核酸递送及免疫调节中的治疗应用。

Result: 外泌体因其固有的生物相容性、内容物保护能力及肿瘤趋向性，在生物传感和药物递送方面优于合成纳米载体。然而，目前主要障碍在于囊泡的非均一性、分离/定量方法的变异性以及缺乏标准化流程。文章还评估了当前的实践性能标准、报告优先级及监管考量，强调了在队列研究中验证 assays 的重要性。

Conclusion: 未来的发展趋势包括利用人工智能辅助分析和开发便携式即时检测（POCT）设备，以加速可临床部署的外泌体平台的开发。通过解决标准化和 reproducibility 问题，整合样本处理与数据分析，外泌体技术将在肺癌的早期筛查、治疗监测及复发检测中发挥关键的临床应用价值。

Abstract: Lung cancer is the leading cause of cancer-related mortality worldwide. It is frequently diagnosed at an advanced stage and exhibits significant morphological and molecular heterogeneity, encompassing both small- and non-small-cell types, as well as major histological variations. Conventional imaging and tissue biopsies are invasive and may fail to reflect dynamic tumor biology. Exosomes (30-150 nm, extracellular vesicles that are products of the endosome) are present in plasma, serum, saliva, and urine and contain tumor-reflective proteins, lipids, and RNAs. This review summarizes the strategies used in biosensors to detect exosomes in lung cancer using various approaches, including electrochemical, optical, microfluidic, and nanomaterial-assisted biosensors, which detect various markers, such as PD-L1, EGFR, and oncogenic miRNAs/lncRNAs. Exosomes possess inherent biocompatibility, intrinsic cargo protection, and the ability to target tumors (homing capabilities), attributes that synthetic nanocarriers, such as liposomes, polymeric, and inorganic nanoparticles, lack. These characteristics render exosomes advantageous for biosensing and delivery applications. We also discuss exosome-enabled therapeutic directions, such as drug- and nucleic-acid-loaded exosomes and immunomodulatory cargos, and indicate the new clinical-study environment. The most important obstacles are the non-uniformity of vesicles, variability in isolation/quantification, and lack of standardization. The discussion concludes with an examination of the practice performance standards, reporting priorities, and emerging trends. These include AI-assisted analysis and the use of portable point-of-care devices, which aim to expedite the development of clinically deployable exosome platforms for lung cancer diagnosis. We examined the integration of a sample into an answer, validation of assays on cohorts, and conceptual implications of regulatory considerations for reproducible measurements. This approach facilitates practical applications, such as screening, treatment monitoring, and early relapse detection.

</details>


### [35] [Retrorsine-induced hepatotoxicity is mediated by inhibition of the EGFR/AKT/c-Jun axis and disruption of calcium homeostasis in primary hepatocytes.](https://pubmed.ncbi.nlm.nih.gov/41539484/)
*Congcheng Zhang,Qin Zhang,Pan Wang,Hai Yin,Yiling Liu,Shuhang Zhang,Hao Lu*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本研究结合网络毒理学、分子对接及功能实验，揭示了吡咯里西啶生物碱（PA）中的 retrorsine 通过抑制 EGFR/AKT/c-Jun 信号轴并引发钙超载，导致内质网应激相关的自噬和肝细胞死亡，从而诱发肝窦阻塞综合征（HSOS）。


<details>
  <summary>Details</summary>
Motivation: 吡咯里西啶生物碱（PAs）是常见的天然毒素，其中 retrorsine 具有强肝毒性，摄入含 PA 产品是导致肝窦阻塞综合征（HSOS）的主要原因。目前亟需阐明 retrorsine 诱导肝毒性的具体分子机制，以寻找潜在的治疗靶点。

Method: 采用多学科联合策略：1. 网络毒理学分析筛选 HSOS 相关靶点及富集通路；2. 分子对接技术预测相互作用；3. 在原代大鼠肝细胞中进行功能验证，检测关键蛋白（p-AKT, p-EGFR, p-c-Jun）的表达水平变化。

Result: 1. 鉴定出 136 个与 HSOS 相关的靶点，主要富集在钙信号通路和 PI3K-AKT 通路，关键靶点包括 AKT1、JUN 和 EGFR。2. 实验证实 retrorsine 显著降低原代肝细胞中 p-AKT、p-EGFR 和 p-c-Jun 的水平（p < 0.05）。3. 机制上，retrorsine 抑制 EGFR/AKT/c-Jun 生存轴，触发细胞内钙超载，进而引起内质网应激相关的自噬及肝细胞死亡。

Conclusion: 研究阐明了 retrorsine 诱导肝细胞损伤的新机制，即通过抑制 EGFR/AKT/c-Jun 轴导致细胞死亡。该通路作为潜在的治疗靶点，为 PA 诱导的 HSOS 提供了新的干预策略。

Abstract: Pyrrolizidine alkaloids (PAs) occur in approximately 3 %-5 % of flowering plants and are common natural toxins; retrorsine is among the most hepatotoxic. Ingestion of PA-containing products is a major cause of hepatic sinusoidal obstruction syndrome (HSOS). To elucidate the mechanism of retrorsine-induced hepatotoxicity, we combined network toxicology, molecular docking, and functional assays in primary hepatocytes. We identified 136 HSOS-related targets enriched in calcium signaling and PI3K-AKT pathways, including AKT1, JUN, and EGFR. In primary rat hepatocytes, retrorsine significantly decreased p-AKT, p-EGFR, and p-c-Jun levels (p < 0.05). Together, these data suggest that retrorsine inhibits EGFR/AKT/c-Jun axis and triggers intracellular calcium overload, leading to ER stress-associated autophagy and hepatocyte death. These findings reveal a mechanism of retrorsine-induced hepatocyte injury and highlight the EGFR/AKT/c-Jun survival pathway as a potential therapeutic target in PA-induced HSOS.

</details>


### [36] [Proteomic signatures in triple-negative breast cancer.](https://pubmed.ncbi.nlm.nih.gov/41512917/)
*Sohit Kashyap,Vishal Patidar,Anil Kumar,Prachi Sahu,Monisha Dhiman,Pardeep Garg,Aklank Jain,Anjana Munshi*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本综述系统阐述了蛋白质组学在三阴性乳腺癌（TNBC）研究中的关键作用，重点分析了基于肿瘤组织、血清及外泌体的蛋白质生物标志物在诊断、预后预测及治疗反应评估中的应用。文章强调了将蛋白质组学与转录组学、基因组等多组学技术结合，对于推动TNBC精准医疗、实现患者风险分层及开发个性化治疗方案的重要意义，并展望了单细胞蛋白质组学等新兴技术的未来潜力。


<details>
  <summary>Details</summary>
Motivation: 三阴性乳腺癌（TNBC）具有高度侵袭性和异质性，且缺乏雌激素受体、孕激素受体和HER2等明确的治疗靶点，导致目前主要依赖化疗，患者预后较差且复发率高。鉴于现有治疗手段有限，亟需通过高通量技术识别新的生物标志物以改善诊断、预后评估及治疗策略，从而解决TNBC临床管理中的未满足需求。

Method: 本文采用文献综述的方法，综合分析了既往发现、当前临床应用及未来机遇。重点梳理了来自肿瘤组织、血清和外泌体蛋白质组学的关键蛋白特征，特别是涉及免疫检查点、细胞表面糖蛋白及肿瘤微环境调节因子的生物标志物。同时，探讨了蛋白质组学与转录组学、基因组学等其他组学技术的整合分析方法，以及单细胞蛋白质组学和整合免疫蛋白质基因组学等新兴技术手段。

Result: 研究确认了多种具有潜力的蛋白质生物标志物，能够预测化疗反应和疾病进展。这些标志物有助于TNBC的亚型分类和风险分层。通过多组学整合，深化了对TNBC病理生理机制的理解，并促进了精准医疗方法的发展。综述还指出了经过临床验证和新开发的生物标志物在诊断、预后和治疗干预中的具体应用价值。

Conclusion: 蛋白质组学已成为重塑TNBC生物学理解、亚型分类及临床管理的核心支柱。通过转化新一代蛋白质组学技术，结合多组学数据，能够推动TNBC向精准医疗转变，助力个性化治疗、药物重定位及合理联合疗法的开发。该领域的发展有望彻底改变TNBC的生物标志物发现模式及临床诊疗策略。

Abstract: An aggressive and heterogeneous malignancy, referred to as triple-negative breast cancer, is characterised by the absence of estrogen receptor, progesterone receptor, and human epidermal growth factor receptor 2. Treatment options remain limited, relying primarily on chemotherapy due to the lack of well-defined therapeutic targets, which is linked with poor prognosis and high recurrence rates. Proteomics and other high-throughput technologies have significantly advanced TNBC research by enabling the identification of protein-based biomarkers with potential applications in diagnosis, prognosis, and treatment. Through protein biomarkers that affect immune checkpoints, cell-surface glycoproteins, and regulators of tumor microenvironment interactions, key protein signatures from tumor tissue, serum, and exosomal proteomics have been found to have the potential to predict chemotherapy response and disease progression. To develop new therapeutic approaches, these biomarkers are being investigated. Combining proteomics with other omics technologies, such as transcriptomics and genomics, enables the development of precision medicine approaches and provides deeper insights into the pathophysiology of TNBC. Clinically validated and newly developed protein biomarkers for diagnosis, prognosis, and treatment interventions are described in this review. The molecular mechanistic aspects have also been discussed. These biomarkers have the potential to aid in the classification, risk stratification, and development of personalized treatment approaches for TNBC. SIGNIFICANCE STATEMENT: Triple-negative breast cancer (TNBC) remains a highly aggressive and heterogeneous form of breast cancer, with very few treatment strategies. This review synthesizes past discoveries, current clinical applications, and future opportunities of proteomics in TNBC, making it highly relevant to the theme of this Special Issue on "Past, Present and Future of Proteomics." By consolidating evidence from human and other preclinical studies, it highlights how proteomic signatures have already transformed our understanding of TNBC biology and subtype classification, while also outlining their growing impact as diagnostic, prognostic, and therapeutic markers. Importantly, the review emphasizes the translational shift enabled by next-generation proteomic technologies to redefine precision medicine for TNBC. It showcases how proteomics can facilitate personalized medicine, drug repurposing, and rational combination therapies, and describes novel avenues such as single-cell proteomics and integrative immunoproteogenomics that are driving the field forward. Thus, this work not only consolidates what has been achieved but also provides perspectives on emerging technologies and innovative applications that could revolutionize biomarker discovery and clinical management of TNBC. It highlights proteomics as a critical pillar in shaping the future of cancer diagnostics and therapeutics, directly aligning with the scope and intent of the special issue.

</details>


### [37] [Garcinol Promotes Recovery After Spinal Cord Injury in Mouse by Modulating Microglial Polarization Through EGFR/PI3K/AKT Signaling.](https://pubmed.ncbi.nlm.nih.gov/41380138/)
*Bin Dai,Xintian Ding,Hongxing Zhang,Shuai Huang,Lei Ye,Rui Chen,Xuansen Weng,Shishuang Gu,Hai Wen*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 本研究通过体内外实验证实，藤黄果醇（Garcinol）能通过抑制EGFR/PI3K/AKT信号通路，促进小胶质细胞向抗炎表型极化，从而减轻脊髓损伤后的继发性炎症并促进神经功能恢复。


<details>
  <summary>Details</summary>
Motivation: 脊髓损伤（SCI）后的继发性炎症主要由过度激活的小胶质细胞驱动，会加剧神经功能缺损。藤黄果醇虽具有抗炎、抗氧化和神经保护特性，但其在SCI治疗中的 efficacy 及具体分子机制尚未被探索。

Method: 采用体外LPS刺激的BV2小胶质细胞模型和体内小鼠T10-T11挫伤性SCI模型。通过评估细胞因子释放、极化标志物及增殖情况，结合运动、膀胱和步态功能评分评价疗效。利用网络药理学、分子对接、转录组分析及EGFR/PI3K/AKT通路调节剂进行机制验证。

Result: 藤黄果醇在体外减少促炎因子（TNF-α, IL-6）和M1标志物，增加M2标志物，促进抗炎极化。在体内，每日腹腔注射20 mg/kg藤黄果醇持续28天，显著抑制小胶质细胞激活和神经炎症，减轻组织损伤，改善 locomotor (BMS评分) 和膀胱功能。机制上确认其作用通过抑制EGFR/PI3K/AKT通路介导。

Conclusion: 藤黄果醇通过调节EGFR/PI3K/AKT通路调控小胶质细胞极化，缓解SCI后继发性炎症并增强神经恢复，具备作为SCI多靶点治疗候选药物的转化潜力。

Abstract: STUDY DESIGN: Preclinical experimental study.

OBJECTIVE: To investigate the therapeutic potential of garcinol in spinal cord injury (SCI), focusing on its regulation of microglial polarization and underlying molecular mechanisms.

SUMMARY OF BACKGROUND DATA: Secondary inflammation following SCI, primarily driven by overactivated microglia, exacerbates neurological deficits. Garcinol, a natural polyisoprenylated benzophenone, exhibits anti-inflammatory, antioxidant, and neuroprotective properties, but its efficacy in SCI has not been explored.

MATERIALS AND METHODS: In vitro, BV2 microglial cells were stimulated with LPS to mimic the SCI inflammatory microenvironment and treated with garcinol to assess cytokine release, polarization, and proliferation. In vivo, T10-T11 contusive SCI was induced in mice, followed by garcinol administration. Functional recovery was evaluated using motor, bladder, and gait assessments. Network pharmacology, molecular docking, and transcriptomic analyses were performed to identify key targets and pathways, with mechanistic validation using EGFR/PI3K/AKT modulators.

RESULTS: In vitro, garcinol promoted microglial polarization toward an anti-inflammatory phenotype by reducing proinflammatory cytokines (TNF-α, IL-6), suppressing M1 markers (iNOS, CD86), and enhancing M2 markers (Arg-1, CD163). Critically, in a mouse model of contusive SCI, daily garcinol administration (20 mg/kg, i.p., 28 d) significantly attenuated microglial activation and neuroinflammation, mitigated secondary tissue damage, which consequently promoted functional recovery, as evidenced by improved locomotor (BMS score) and bladder function. Network and mechanistic studies identified that these beneficial effects were mediated through the inhibition of the EGFR/PI3K/AKT pathway. Collectively, these findings underscore the translational potential of garcinol as a multitarget therapeutic candidate for SCI.

CONCLUSION: Garcinol alleviates secondary inflammation and enhances neurological recovery after SCI by modulating microglial polarization through the EGFR/PI3K/AKT pathway. These findings provide a theoretical and experimental basis for developing garcinol as a multitarget therapeutic candidate for SCI.

</details>


### [38] [Efficacy and Safety of Dotinurad in Patients with Advanced Chronic Kidney Disease.](https://pubmed.ncbi.nlm.nih.gov/40835469/)
*Teppei Motomura,Mayuka Higashi,Akihisa Hattori,Ryosuke Akiyama,Hirayasu Kai*

Source: PubMed

Main category: EGFR Target Biology

TL;DR: 这项单中心回顾性观察研究评估了新型促尿酸排泄药Dotinurad在晚期慢性肾脏病（CKD G3b-G5）高尿酸血症患者中的疗效与安全性。结果显示，治疗6个月后血清尿酸水平显著降低，而估算肾小球滤过率（eGFR）无显著变化，且未观察到不良事件，表明Dotinurad在此类患者中安全有效。


<details>
  <summary>Details</summary>
Motivation: 高尿酸血症与心血管疾病及慢性肾脏病（CKD）密切相关。尽管Dotinurad是一种新型促尿酸排泄药物，但此前尚未在晚期CKD（CKD G3b-G5）患者的临床试验中进行评估。因此，本研究旨在填补这一空白，评估该药物在此特定高危人群中的降尿酸 efficacy、安全性以及对肾功能的影响。

Method: 研究采用单中心回顾性观察设计。纳入14名在门诊接受Dotinurad治疗的CKD G3b至G5期患者，并收集其治疗前后各6个月的数据。主要评估指标包括治疗6个月后的血清尿酸水平变化，以及治疗前后6个月的估算肾小球滤过率（eGFR）。

Result: 治疗6个月后，患者血清尿酸水平从基线的8.2 (8.0-8.5) mg/dL显著降至6.2 (5.9-7.2) mg/dL (p<0.01)。肾功能方面，治疗前6个月eGFR为25.4 (19.4-40.6) mL/min/1.73 m²，治疗时为24.9 (18.3-35.5) mL/min/1.73 m²，治疗后6个月为24.4 (19.2-36.4) mL/min/1.73 m²，差异无统计学意义。此外，研究期间未观察到任何不良事件。

Conclusion: Dotinurad能够安全有效地降低晚期慢性肾脏病（CKD G3b-G5）高尿酸血症患者的血清尿酸水平，且在观察期内未对肾功能产生显著负面影响，也未引发不良事件。

Abstract: Objective Hyperuricemia is associated with cardiovascular disease and chronic kidney disease (CKD). Dotinurad is a new uricosuric drug that has not been evaluated in clinical trials in patients with advanced CKD (CKD G3b-G5). This study evaluated the efficacy of dotinurad in lowering serum uric acid levels, its safety profile, and its effect on the renal function in hyperuricemic patients with CKD G3b to G5. Methods This was a single-center retrospective observational study. Patients Fourteen patients with CKD G3b to G5 who received dotinurad in the outpatient clinic and for whom data were available for six months before and after treatment were enrolled in this study. Serum uric acid levels six months after treatment and the estimated glomerular filtration rate (eGFR) six months before and after treatment were evaluated. Results Serum uric acid levels significantly decreased from 8.2 (8.0-8.5) mg/dL to 6.2 (5.9-7.2) mg/dL six months after treatment (p<0.01). The eGFR was 25.4 (19.4-40.6) mL/min/1.73 m2 6 months before treatment and 24.9 (18.3-35.5) mL/min/1.73 m2 at the time of treatment, but 24.4 (19.2-36.4) mL/min/1.73 m2 at 6 months after treatment, with no significant change noted. No adverse events were observed. Conclusion Dotinurad can safely lower serum uric acid levels in hyperuricemic patients with advanced CKD.

</details>


<div id='CEACAM5 Antibody Therapeutics'></div>

# CEACAM5 Antibody Therapeutics [[Back]](#toc)

### [39] [B7-H3 Gene Expression Shapes Prognosis and Therapeutic Opportunities Across Prostate Cancer Patient Groups.](https://pubmed.ncbi.nlm.nih.gov/42012455/)
*Allison Makovec,Ava P Gustafson,Nishant Gandhi,Swati Rampalli,Ali T Arafa,Andrew Elliott,Norm Smith,Martin Felices,Philippa R Kennedy,Eugene Shenderov,Akash Patnaik,Vivek Narayan,Elisabeth I Heath,Nicholas A Zorko,Xiaolei Shi,Emmanuel S Antonarakis,Rana R McKay,Justin Hwang*

Source: PubMed

Main category: CEACAM5 Antibody Therapeutics

TL;DR: 本研究通过分析8,157例前列腺癌样本，证实B7-H3在不同疾病阶段和人群中广泛表达，支持其作为治疗靶点的可行性。高B7-H3表达与AR相关分子特征及TROP2等表面抗原相关，提示联合靶向策略潜力；其预后价值因肿瘤部位而异（原发灶/激素敏感型预示不良，转移灶预示较好），且亚裔/太平洋岛民转移性患者生存率最低。


<details>
  <summary>Details</summary>
Motivation: B7-H3 (CD276) 是高危局部晚期和治疗难治性转移性前列腺癌（PC）中一个有前景的治疗靶点。为了指导治疗开发和策略制定，需要全面评估前列腺肿瘤中B7-H3的表达情况、分子特征及其对总生存期（OS）的影响，并综合考虑组织部位、激素敏感性状态和种族差异。

Method: 研究分析了8,157例具有配对DNA/RNA数据的前列腺癌样本，根据组织部位、自我报告的种族以及疾病状态（激素敏感性HSPC、去势抵抗性CRPC或神经内分泌前列腺癌NEPC）进行标注。将B7-H3表达分为高表达（>75th百分位）和低表达（<25th百分位）。使用Kaplan-Meier曲线和Cox比例风险模型评估总生存期（OS）。

Result: 1. B7-H3表达广泛存在，但随肿瘤部位、激素敏感性和种族变化。
2. 高表达与AR相关转录因子（HOXB13, FOXA1）、AR相关病理失调（AR-V7, SPOP等）及可操作表面抗原（TROP2, NECTIN-4）一致；与谱系可塑性调节因子（EZH2等）及NEPC相关抗原（DLL3等）相关性弱。
3. 原发性肿瘤和HSPC中高B7-H3表达预示不良OS（HR分别为1.342和1.30）；而在转移性肿瘤中预示较好OS（HR 0.823）。
4. CRPC和NEPC中OS无显著差异，但种族间存在差异，亚裔/太平洋岛民转移性PC患者生存率最差（HR = 3.72）。

Conclusion: B7-H3在各种前列腺癌环境中的维持表达支持其作为治疗靶点的可行性。其与AR相关分子因子、表面抗原以及细胞疗法或抗体药物偶联物（ADC） investigational targets 的关联，提示了潜在的双重靶向治疗策略。

Abstract: PURPOSE: B7-H3 (CD276) represents a promising therapeutic target tested in high-risk localized and treatment-refractory metastatic prostate cancer (PC). To guide therapeutic development and treatment strategies, we examined prostate tumors and evaluated expression, molecular features, and overall survival (OS), accounting for tissue site, hormone-sensitivity status, and race.

EXPERIMENTAL DESIGN: 8,157 PC samples with paired DNA/RNA were analyzed based on annotations by tissue site, self-reported race, and disease state: hormone-sensitive (HSPC), castration-resistant (CRPC), or neuroendocrine PC (NEPC). Expression quartiles were B7-H3-high (>75th percentile) or B7-H3-low (<25th percentile). OS was evaluated using Kaplan-Meier and Cox proportional hazards models.

RESULTS: B7-H3 expression was broadly maintained but varied by tumor site, hormone-sensitivity status, and race. High expression aligned with AR-associated transcription factors (HOXB13, FOXA1), AR-associated pathogenic dysregulations (AR-V7, SPOP, FOXA1, TMPRSS2:ERG fusions), and actionable surface antigens (TROP2, NECTIN-4). Weak correlations were found for lineage-plastic program regulators (EZH2, SOX2, ASCL1) and NEPC-associated surface antigens (DLL3, CEACAM5). High B7-H3 expression in primary tumors and HSPCs portended adverse OS (HR: 1.342, 1.30, CI: 1.19-1.512, 1.15-1.46, q < 0.0001), although favorable in metastatic tumors (HR: 0.823, CI: 0.719-0.942, q = 0.0048). No significant differences in OS were observed among CRPCs and NEPCs, although OS varied by race, with poorest survival in Asian/Pacific Islander metastatic PC patients (HR = 3.72, CI: 1.49-9.29, q = 0.012).

CONCLUSIONS: Maintained B7-H3 expression in various PC settings supports its viability as a target. Associations with AR-related molecular factors, surface antigens, and investigative targets for cell therapy or antibody-drug conjugates (ADC) suggest potential dual-targeting strategies.

</details>


### [40] [Antibody-Drug Conjugates in Small-Cell Lung Cancer: DLL3, B7-H3, TROP2 and Beyond.](https://pubmed.ncbi.nlm.nih.gov/41985715/)
*Federico Monaca,Igor Gomez-Randulfe,Javier Torres-Jiménez,Muskan Agarwal,Stephen V Liu,Luis Paz-Ares,Raffaele Califano*

Source: PubMed

Main category: CEACAM5 Antibody Therapeutics

TL;DR: 本综述探讨了抗体药物偶联物（ADCs）在广泛期小细胞肺癌（ES-SCLC）治疗中的最新进展，重点分析了针对DLL3、TROP2等靶点的临床数据、耐药机制、生物标志物局限性以及联合治疗策略。


<details>
  <summary>Details</summary>
Motivation: 广泛期小细胞肺癌（ES-SCLC）预后极差，尽管化疗联合免疫疗法已应用，但生存结果仍不理想，且二线治疗选择有限。ADCs作为新兴疗法显示出潜力，但其有效实施面临原发/获得性耐药及生物标志物局限等重大挑战，亟需系统梳理现有数据以指导未来方向。

Method: 采用叙述性综述方法，总结ADCs的初步临床数据，深入分析靶点生物学特性、关键分子设计特征、中枢神经系统渗透潜力及安全性概况；同时评估与免疫检查点抑制剂、DNA损伤反应抑制剂的联合策略，以及与DLL3定向T细胞衔接器的相互作用和最佳排序。

Result: 针对DLL3、TROP2、B7-H3和SEZ6等靶点的ADCs在二线及后线治疗中显示出令人鼓舞的抗肿瘤活性；早期针对CEACAM5、PTK7等抗原的药物也在开发中。然而，耐药性和患者选择/监测的生物标志物不足仍是主要障碍。

Conclusion: ADCs为ES-SCLC提供了新的治疗希望，特别是在后线设置中。未来的发展方向包括优化分子设计以改善中枢渗透性，克服耐药性，开发更有效的生物标志物，并探索与现有标准护理及其他新兴疗法（如T细胞衔接器）的最佳联合与排序策略。

Abstract: Extensive-stage small-cell lung cancer (ES-SCLC) is an aggressive form of lung cancer with a dismal prognosis, and despite the advent of chemo-immunotherapy, survival outcomes remain poor, with limited second-line options available. Antibody-drug conjugates (ADCs) have emerged as an innovative therapeutic class in this context, leveraging tumour-associated antigens to deliver cytotoxic payloads with improved selectivity. ADCs targeting DLL3, TROP2, B7-H3, and SEZ6, together with earlier-phase agents directed against CEACAM5, PTK7, and other emerging antigens, are advancing through clinical development and have demonstrated encouraging antitumour activity in second- and later-line settings. Primary and acquired resistance together with the limitations of current biomarkers for patient selection and response monitoring remain the major challenges to effective ADC implementation. This narrative review summarises preliminary clinical data on ADCs, with particular emphasis on target biology, key molecular design features, potential central nervous system penetration, and the emerging safety profile. It also evaluates combination strategies with immune checkpoint and DNA-damage response inhibitors, the interplay and optimal sequencing with DLL3-directed T-cell engagers, and existing standards of care and future directions.

</details>


### [41] [Antibody-Drug Conjugates in Gastrointestinal Oncology: Clinical Efficacy and Inpatient Toxicity Management.](https://pubmed.ncbi.nlm.nih.gov/42042562/)
*Ashish Sharma,Harendra Kumar,Ruchir Paladiya,Rajvardhan Sisodia,Hareesha Rishab Bharadwaj,Islam Mohamed,Saqr Alsakarneh,Umar Hayat,Sneh Sonaiya,Hema Sameera Pinnam,Hassam Ali,Dushyant Singh Dahiya*

Source: PubMed

Main category: CEACAM5 Antibody Therapeutics

TL;DR: 本文综述了抗体偶联药物（ADCs）在晚期胃肠道癌症治疗中的进展，重点分析了曲妥珠单抗德鲁替康（T-DXd）的疗效及针对CLDN18.2、TROP2等靶点的新兴疗法。文章强调了ADCs特有的毒性谱，特别是间质性肺病（ILD）的风险，并指出医院系统需建立多学科协作机制以应对住院患者的毒性管理挑战，倡导基于生物标志物的个性化医疗。


<details>
  <summary>Details</summary>
Motivation: 随着ADCs在胃肠道肿瘤治疗中的地位日益重要（如从二线治疗向前线推进及适应症扩大），其特有的严重毒性（尤其是ILD）给临床管理带来挑战。现有研究缺乏对住院环境下ADC相关毒性识别、鉴别诊断及机构应对策略的系统性分析，因此需要明确医院系统如何适应这一变化以确保患者安全。

Method: 采用叙述性综述方法，系统分析ADCs在胃肠道肿瘤学中的作用机制、临床有效性数据（重点关注HER2阳性胃癌/GEJ癌）以及安全性特征。同时评估当前临床实践中对于ADC相关毒性（特别是ILD、血液学抑制等）的管理现状，并提出机构层面的应对框架。

Result: 1. 疗效确认：T-DXd显著提高了经曲妥珠单抗治疗后HER2阳性胃癌/GEJ癌患者的客观缓解率和总生存期，确立了其作为重要后线治疗的地位。
2. 研发趋势：针对CLDN18.2、TROP2和CEACAM5的ADCs正在扩展胃肠道肿瘤的治疗版图。
3. 毒性特征：ADCs具有特定的毒性谱，包括血液学抑制、胃肠道副作用、肝毒性及潜在的致命性间质性肺病（ILD）。
4. 诊断难点：住院患者中，ADC相关毒性与感染或疾病进展的临床表现重叠，导致鉴别诊断困难。

Conclusion: ADCs正推动胃肠道癌症治疗向个性化医疗转变，但其临床应用伴随着显著的毒性风险。为确保安全整合到标准临床实践中，医院系统必须进化，建立快速识别毒性、统一管理协议以及肿瘤科与住院团队之间高效协作的多学科管理体系。强调基于生物标志物的患者选择和个体化毒性监测至关重要。

Abstract: Antibody-drug conjugates (ADCs) are reshaping the therapeutic approach to advanced gastrointestinal cancers by integrating tumor-specific monoclonal antibodies with potent cytotoxic payloads to improve targeted tumor cell destruction while minimizing systemic exposure. Compared to traditional chemotherapy, trastuzumab deruxtecan has significantly improved objective response rates and overall survival in HER2-positive gastric and gastroesophageal junction tumors after trastuzumab-based therapy. This supports its role as an important second-line or later treatment option. The ongoing advancement of ADCs targeting CLDN18.2, TROP2, and CEACAM5 indicates that this therapeutic category will continue to expand across gastrointestinal neoplasms. Nonetheless, these advancements are accompanied by a specific and clinically significant toxicity profile. Hematologic suppression, gastrointestinal side effects, hepatotoxicity, and notably interstitial lung disease (ILD) are essential consequences that may need inpatient assessment and care. Interstitial lung disease (ILD), although uncommon, may be severe or lethal if not identified immediately and treated swiftly with medication cessation and corticosteroids. In hospitalized patients, distinguishing ADC-related toxicity from infection or disease progression is often difficult owing to overlapping clinical manifestations, requiring meticulous evaluation and interdisciplinary cooperation. As ADCs are integrated into earlier treatment lines and across a broader patient population, hospital systems must evolve to ensure prompt identification, consistent management protocols, and efficient collaboration between oncology and inpatient teams. This study analyzes the mechanisms, clinical effectiveness, and safety profile of ADCs in gastrointestinal oncology, pointing out the importance of institutional preparedness to safely incorporate these medicines into standard clinical practice. These features also align ADC therapy with personalized medicine by emphasizing biomarker-guided patient selection and individualized toxicity monitoring.

</details>


### [42] [Carcinoembryonic Antigen-Related Cell Adhesion Molecule 5 as a Biomarker for Predicting Response to Erlotinib and Gefitinib in Lung Adenocarcinoma: An Integrative Analysis of Transcriptomic Data of PC-9 and Drug-Resistant PC-9 Cell Lines.](https://pubmed.ncbi.nlm.nih.gov/41977278/)
*Pritsana Raungrut,Suchanan Tanyapattrapong,Saowanee Maungchanburi,Kanyaphak Bumrungchoo*

Source: PubMed

Main category: CEACAM5 Antibody Therapeutics

TL;DR: 本研究通过整合转录组数据分析，鉴定出CEACAM5为肺腺癌患者对厄洛替尼（ER）和吉非替尼（GB）获得性耐药的预测生物标志物。


<details>
  <summary>Details</summary>
Motivation: 厄洛替尼（ER）和吉非替尼（GB）虽能显著延长晚期肺腺癌患者的中位生存期，但部分患者因产生治疗耐药性而出现早期疾病复发。本研究旨在识别与ER或GB获得性耐药相关的共同基因。

Method: 1. 比较敏感PC-9细胞与ER耐药（PC-9/ER）或GB耐药（PC-9/GB）细胞系的转录组数据，筛选差异表达基因。
2. 利用韦恩图识别共同基因。
3. 在构建的耐药细胞系中验证候选基因（耐药细胞系经6个月诱导，对ER和GB的耐药性分别增加40.7倍和109.2倍）。
4. 通过流式细胞术分析耐药细胞系对细胞周期阻滞和凋亡的反应。
5. 分析基因与蛋白表达的一致性。

Result: 1. PC-9/ER和PC-9/GB细胞系均表现出对ER或GB诱导的细胞周期阻滞和凋亡的耐药性。
2. 整合分析发现KRT13和CEACAM5在两种耐药细胞系中均上调。
3. KRT13和CEACAM5的表达模式与转录组数据不完全一致，仅CEACAM5显示出一致的基因和蛋白表达水平。

Conclusion: CEACAM5可作为预测肺腺癌患者对厄洛替尼和吉非替尼治疗反应的生物标志物。

Abstract: Erlotinib (ER) and gefitinib (GB) are frequently prescribed for patients with advanced lung adenocarcinoma (ADC). Although both ER and GB significantly extend median survival, several patients experience early disease relapse due to treatment resistance. This study aimed to identify common genes associated with acquired resistance to ER or GB. This integrative analysis of transcriptomic data identified differentially expressed genes by comparing sensitive PC-9 cells with ER-resistant (PC-9/ER) or GB-resistant (PC-9/GB) cell lines. A Venn diagram was used to identify common genes. Candidate genes were confirmed in our generated drug-resistant cell lines. After six months, the PC-9/ER and PC-9/GB cell lines exhibited 40.7-fold and 109.2-fold increases in resistance to ER and GB, respectively. Flow cytometric analysis demonstrated that both resistant cell lines were resistant to cell cycle arrest and apoptosis induced by ER or GB. Through integrated analysis, we found that two genes, KRT13 and CEACAM5, were up-regulated in both the PC-9/ER and PC-9/GB cell lines. The expression patterns of these two genes did not align with the transcriptome, with only CEACAM5 exhibiting consistent gene and protein expressions. Our findings indicate that CEACAM5 serves as a biomarker for predicting the response of patients to ER and GB treatments.

</details>
